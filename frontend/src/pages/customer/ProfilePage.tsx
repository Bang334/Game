...
const ProfilePage = () => {
  ...
  const fetchPendingDeposits = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://localhost:3001/api/customer/pending-deposits', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDepositRequest = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:3001/api/customer/deposit-request', {
        amount: parseFloat(depositAmount),
        description: depositDescription || `Nạp ${depositAmount} VNĐ`
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });

      enqueueSnackbar('Yêu cầu nạp tiền đã được gửi! Vui lòng chờ admin duyệt.', { variant: 'success' });
      setDepositAmount('');
      setDepositDescription('');
      fetchPendingDeposits();
    } catch (error: any) {
      console.error('Error creating deposit request:', error);
      const errorMsg = error.response?.data?.error === 'INVALID_AMOUNT'
        ? 'Số tiền không hợp lệ'
        : 'Có lỗi xảy ra khi gửi yêu cầu nạp tiền';
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Card sx={{
        ...
        {/* Tab 5: Balance Transaction History */}
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mb: 4 }}>
                <Card sx={{ flex: 1, minWidth: 200, background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
                  <CardContent>
                    ...
                    <Typography variant="h5" sx={{ color: 'white', fontWeight: 700 }}>
                      {formatPrice(transactionStats.total_deposits || 0)}
                    </Typography>
                  </CardContent>
                </Card>
                ...
                <Card sx={{ flex: 1, minWidth: 200, background: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)' }}>
                  <CardContent>
                    ...
                    <Typography variant="h5" sx={{ color: 'white', fontWeight: 700 }}>
                      {formatPrice(transactionStats.total_purchases || 0)}
                    </Typography>
                  </CardContent>
                </Card>
                ...
                <Card sx={{ flex: 1, minWidth: 200, background: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' }}>
                  <CardContent>
                    ...
                    <Typography variant="h5" sx={{ color: 'white', fontWeight: 700 }}>
                      {transactionStats.transaction_count || 0}
                    </Typography>
                  </CardContent>
                </Card>
              </Box>
                        ...
                        <Box sx={{ textAlign: 'right', minWidth: 150 }}>
                          ...
                          <Box>
                            <Typography variant="caption" color="text.secondary" display="block">
                              Trước: {formatPrice(transaction.balance_before)}
                            </Typography>
                            <Typography variant="caption" color="text.secondary" display="block">
                              Sau: {formatPrice(transaction.balance_after)}
                            </Typography>
                          </Box>
                        </Box>
                      </Box>
