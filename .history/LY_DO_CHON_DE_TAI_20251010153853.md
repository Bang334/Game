...
- Các thuộc tính đặc trưng:
  ...
  + Nhà phát hành (publisher)
  + Chế độ chơi (mode): Single-player, Multiplayer, Co-op, Online, Offline
  + Năm phát hành
  + Giá cả
  + Rating trung bình
...
Hành vi người dùng:
- Purchase (Mua game):
  + Trọng số cao nhất (weight = 5.0)
  + Phản ánh sở thích mạnh mẽ nhất của người dùng
  + Lưu thông tin: user_id, game_id, purchase_date, price
  
- Review (Đánh giá):
  + Trọng số cao (weight = 4.0)
  + Bao gồm rating (1-5 sao) và comment
  + Phản ánh mức độ hài lòng sau khi chơi
  + Lưu thông tin: user_id, game_id, rating, comment, review_date
  
- Wishlist (Yêu thích):
  + Trọng số trung bình (weight = 3.0)
