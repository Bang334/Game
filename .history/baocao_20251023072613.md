...
2.2.2 Numeric Features Processing
Hệ thống xử lý 8 numeric features:
...
**2. Price (Giá tiền):**
```python
normalized_price = log10(price) / 7.0
# Log scale để giảm độ lệch do giá cao
# VD: 500,000 VND → log10(500000)/7 ≈ 0.814
```

**3. Downloads (Lượt tải):**
```python
normalized_downloads = log10(downloads) / 9.0
# Log scale để xử lý số lượng lớn
# VD: 1,000,000 → log10(1000000)/9 ≈ 0.667
```

**4. Capacity (Dung lượng):**
```python
normalized_capacity = capacity_gb / 100.0
# VD: 50GB → 0.50
```

**5. Release Year (Năm phát hành):**
```python
normalized_year = (release_year - 1990) / 35.0
# Baseline: 1990, Range: 35 năm (1990-2025)
# VD: 2023 → (2023-1990)/35 ≈ 0.943
```

**6-7. CPU Score và GPU Score (Hiệu năng):**

Hàm get_cpu_score(cpu_name):
```python
...
# Sau đó normalize:
normalized_cpu = cpu_score / 10000.0
```

Ví dụ CPU scores (từ cpu.json):
- Intel Core i9-13900K: 58,000 → 5.8
- Intel Core i7-12700K: 28,000 → 2.8
- Intel Core i5-12400F: 19,500 → 1.95
- Intel Core i3-12100F: 13,000 → 1.3
- AMD Ryzen 9 7950X: 60,000 → 6.0
- AMD Ryzen 7 5800X: 28,500 → 2.85
- AMD Ryzen 5 5600X: 24,000 → 2.4

Hàm get_gpu_score(gpu_name):
```python
...
# Sau đó normalize:
normalized_gpu = gpu_score / 10000.0
```

Ví dụ GPU scores (từ gpu.json):
- NVIDIA RTX 4090: 36,000 → 3.6
- NVIDIA RTX 4070: 19,000 → 1.9
- NVIDIA RTX 3060: 8,500 → 0.85
- NVIDIA RTX 3050: 6,000 → 0.6
- AMD RX 7900 XTX: 24,000 → 2.4
- AMD RX 6600 XT: 7,500 → 0.75
- Intel UHD Graphics 770: 1,200 → 0.12

**8. RAM (Bộ nhớ):**
```python
# Parse từ string
ram_gb = int(ram_string.replace('GB', ''))
normalized_ram = ram_gb / 32.0
# VD: 16GB → 0.50
```

2.2.3 Feature Combination
```python
# Text features: TF-IDF matrix (N_games × 1000)
text_features = tfidf_vectorizer.fit_transform(text_data)

# Numeric features: Normalized matrix (N_games × 8)
numeric_features = [
    [normalized_rating, normalized_price, normalized_downloads,
     normalized_capacity, normalized_year, normalized_cpu,
     normalized_gpu, normalized_ram]
    for game in games
]

# Kết hợp với trọng số khác nhau
combined_features = hstack([
    text_features * 1.0,      # Text features: weight × 1.0
    numeric_features * 0.5     # Numeric features: weight × 0.5
])
...
**Lý do trọng số Text > Numeric (1.0 vs 0.5):**
-	Text features (genre, publisher) quan trọng hơn cho việc xác định game tương tự
-	Numeric features (price, downloads) chỉ là yếu tố phụ
-	VD: 2 game cùng genre "RPG" quan trọng hơn 2 game cùng giá 500k

2.2.4 CPU/GPU Benchmark Data
Hệ thống sử dụng 2 file JSON để tra cứu điểm benchmark:

**cpu.json** - 67 CPU models:
- Intel: i9 (35k-58k), i7 (17k-48k), i5 (12k-38k), i3 (8k-13k)
- AMD: Ryzen 9 (39k-60k), Ryzen 7 (22k-38k), Ryzen 5 (13k-32k)
- Generic mappings cho CPU không có trong database

**gpu.json** - 80+ GPU models:
- NVIDIA RTX 40 series: 13k-36k
- NVIDIA RTX 30 series: 6k-19k
- NVIDIA RTX 20 series: 7k-13k
- NVIDIA GTX series: 800-9.5k
- AMD RX 7000 series: 9k-24k
- AMD RX 6000 series: 4k-15k
- Integrated graphics: 500-1.8k

**Fallback mechanism:**
Nếu không tìm thấy exact match, tìm generic:
- "Intel i7-12700K" → tìm "Intel i7" → 24,000
- "AMD Ryzen 5 5600X" → tìm "AMD Ryzen 5" → 18,000

2.2.5 Ví dụ minh họa: So sánh 2 games

**Game A: Cyberpunk 2077**
```
Text Features:
...
- Description: "Cyberpunk 2077 is an open-world action RPG..."
...
- Capacity: 70 GB
- Release Year: 2020
- Min Spec CPU: "Intel Core i7-6700"
- Min Spec GPU: "GTX 1060"
- Min Spec RAM: "12GB"

Numeric Features (Normalized):
- rating: 4.2/5 = 0.84
- price: log10(1299000)/7 = 0.87
- downloads: log10(5000000)/9 = 0.74
- capacity: 70/100 = 0.70
- year: (2020-1990)/35 = 0.86
- cpu_score: 17000/10000 = 1.7
- gpu_score: 4200/10000 = 0.42
- ram: 12/32 = 0.375

Feature Vector: [0.84, 0.87, 0.74, 0.70, 0.86, 1.7, 0.42, 0.375]
```

**Game B: The Witcher 3**
```
Text Features:
- Genre: ["Action", "RPG", "Open World"]
- Publisher: "CD Projekt Red"
...
- Mode: "Single-player"
- Language: ["English", "Vietnamese", "Japanese"]
- Description: "The Witcher 3: Wild Hunt is an action RPG..."
...
- Capacity: 50 GB
- Release Year: 2015
- Min Spec CPU: "Intel Core i5-2500K"
- Min Spec GPU: "GTX 660"
- Min Spec RAM: "8GB"

Numeric Features (Normalized):
- rating: 4.9/5 = 0.98
- price: log10(799000)/7 = 0.84
- downloads: log10(10000000)/9 = 0.78
- capacity: 50/100 = 0.50
- year: (2015-1990)/35 = 0.71
- cpu_score: 15000/10000 = 1.5
- gpu_score: 2800/10000 = 0.28
- ram: 8/32 = 0.25

Feature Vector: [0.98, 0.84, 0.78, 0.50, 0.71, 1.5, 0.28, 0.25]
```

**Content Similarity Calculation:**
```python
# Text similarity (TF-IDF + Cosine)
# Cùng genre: "Action RPG Open World"
# Cùng publisher: "CD Projekt Red"
# Cùng platforms: ["Windows", "PlayStation", "Xbox"]
# → Text similarity ≈ 0.95 (rất cao)

# Numeric similarity (Euclidean distance → Cosine)
# Vector A: [0.84, 0.87, 0.74, 0.70, 0.86, 1.7, 0.42, 0.375]
# Vector B: [0.98, 0.84, 0.78, 0.50, 0.71, 1.5, 0.28, 0.25]
# → Numeric similarity ≈ 0.78 (tương đối cao)

# Combined similarity
similarity = (text_sim × 1.0 + numeric_sim × 0.5) / 1.5
           = (0.95 × 1.0 + 0.78 × 0.5) / 1.5
           = (0.95 + 0.39) / 1.5
           = 1.34 / 1.5
           = 0.89 (89% similarity)
```
...
**Kết luận:**
Cyberpunk 2077 và The Witcher 3 có similarity score 0.89 (rất cao) vì:
✅ Cùng publisher (CD Projekt Red)
✅ Cùng genres (Action, RPG, Open World)
✅ Cùng platforms
✅ Price range tương đương
✅ System requirements tương tự
→ Hệ thống sẽ gợi ý The Witcher 3 cho user thích Cyberpunk 2077 và ngược lại
