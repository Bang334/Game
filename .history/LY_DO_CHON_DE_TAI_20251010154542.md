...
8.3.5 Ensemble và Dynamic Weighting

Hệ thống kết hợp 4 phương pháp:
...
- Dựa trên keywords từ description, tags, reviews
- Sử dụng TF-IDF similarity
- Ưu tiên cao khi user tìm kiếm bằng keyword

Dynamic Weight Adjustment:
- Tự động chuyển đổi trọng số khi có/không có keyword
...
- Temporal decay được áp dụng lên weighted interactions
...
- Endpoint chính:
  + GET /recommend/user_id: Gợi ý cho user
  + POST /recommend: Gợi ý với keyword
  + GET /users: Danh sách users
  + GET /user/user_id/profile: User profile và preferences
- Input: user_id, keyword (optional), num_recommendations (default=10)
- Output: JSON với game recommendations và scores

Data Storage:
- JSON files cho game, user, CPU, GPU data
- SQLite database (user_interactions.db) cho tracking:
  + Lưu lịch sử interactions (view, purchase, favorite)
  + Timestamps cho temporal analysis
  + Schema: user_id, game_id, interaction_type, timestamp, rating

Temporal Impact Implementation:
- Áp dụng exponential decay cho interactions cũ
- Công thức: weight × exp(-decay_rate × days_elapsed)
- Mặc định decay_rate = 0.01
- Chỉ tính interactions trong 365 ngày gần nhất

User Preference Analysis:
- Tự động phân tích sở thích từ interactions
- Xác định:
  + Top preferred publishers
  + Top preferred genres  
  + Top preferred platforms
  + Preferred keywords
...
- Matplotlib charts cho phân tích:
  + User-game relationship heatmap
