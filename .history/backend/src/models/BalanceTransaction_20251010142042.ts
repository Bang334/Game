...
export const BalanceTransactionModel = {
  async create(data: {
    ...
    transfer_proof_image?: string
    transfer_amount?: number
    transfer_reference?: string
  }): Promise<number> {
    const [result] = await pool.execute<ResultSetHeader>(
      `INSERT INTO balance_transaction 
       (user_id, amount, balance_before, balance_after, transaction_type, status, description, 
        related_game_id, transfer_proof_image, transfer_amount, transfer_reference)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        data.user_id,
        data.amount,
        ...
        data.transfer_proof_image || null,
