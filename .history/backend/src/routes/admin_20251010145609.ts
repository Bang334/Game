...
router.get('/reviews', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const reviews = (rows as any[]).map(row => ({
      ...
      game: {
        title: row.game_name,  // Frontend expects 'title'
        image_url: row.game_image,  // Frontend expects 'image_url'
        game_id: row.game_id  // Add game ID for reference
      }
    }))
...
router.get('/games', requireAdmin, async (req: Request, res: Response) => {
  try {
    const [rows] = await pool.execute(`
      SELECT g.game_id, g.name, g.description, g.price, g.image, g.link_download,
             g.release_date, g.mode, g.multiplayer, g.capacity, g.age_rating, 
             g.average_rating, g.downloads,
