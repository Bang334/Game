...
// Purchase Management
router.get('/purchases', requireAdmin, async (_req: Request, res: Response) => {
  try {
...
router.get('/reviews', requireAdmin, async (_req: Request, res: Response) => {
  try {
    const [rows] = await pool.query(`
      SELECT r.*, u.username, g.name as game_name
      FROM review r
...
// Wishlist Management
router.get('/wishlists', requireAdmin, async (_req: Request, res: Response) => {
  try {
    const [rows] = await pool.query(`
