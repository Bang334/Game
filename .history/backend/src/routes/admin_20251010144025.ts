...
import { pool } from '../db'
import { BalanceTransactionModel, UserModel, PurchaseModel } from '../models'
...
router.get('/users', requireAdmin, async (req: Request, res: Response) => {
  try {
    const [rows] = await pool.execute(`
      SELECT u.user_id, u.username, u.email, u.balance, u.age, u.gender, 
...
router.get('/purchases', requireAdmin, async (req: Request, res: Response) => {
  try {
    const [rows] = await pool.execute(`
      SELECT p.purchase_id, p.user_id, p.game_id, p.purchase_date,
             u.username, u.email,
             g.name as game_name, g.price as game_price, g.image as game_image
      FROM purchase p
