...
router.get('/games', async (req: Request, res: Response) => {
  try {
    ...
    if (genre) {
      ...
      const gameIds = await GameModel.findByGenre(genreId)
      games = games.filter(game => gameIds.includes(game.game_id))
    }
    
    if (platform) {
      const platformId = parseInt(platform as string)
      const gameIds = await GameModel.findByPlatform(platformId)
      games = games.filter(game => gameIds.includes(game.game_id))
    }
