...
**Tính Content Similarity Score:**
-	Hệ thống dùng **COSINE SIMILARITY** (KHÔNG phải Euclidean Distance!)
-	Công thức: `cosine_sim(A, B) = (A · B) / (||A|| × ||B||)`
-	Cosine similarity CHO TRỰC TIẾP similarity score trong [0, 1]
-	**Content Score = Cosine Similarity** (không cần convert)
-	Trả về games có similarity cao nhất

**Lưu ý quan trọng về Distance vs Similarity:**
```python
# Cosine Similarity (dùng trong hệ thống này)
content_score = cosine_similarity(game_A, game_B)
# Trả về trực tiếp: 0.0 (khác hoàn toàn) đến 1.0 (giống hệt)
# Càng GẦN 1.0 = càng giống

# Euclidean Distance (KHÔNG dùng, chỉ để minh họa vấn đề normalize)
distance = sqrt(sum((A - B)²))
# Trả về: 0 (giống hệt) đến ∞ (khác xa)
# Càng GẦN 0 = càng giống
# Nếu dùng, phải convert: similarity = 1 / (1 + distance)
```

**Tại sao dùng Cosine Similarity thay vì Euclidean?**
1. Cosine đo góc giữa vectors, bỏ qua magnitude (độ lớn)
2. Phù hợp cho high-dimensional data (1008 features)
3. Không bị ảnh hưởng bởi độ dài vector
4. Kết quả luôn trong [0, 1], dễ hiểu
...
**LƯU Ý QUAN TRỌNG:**
- Hệ thống dùng **COSINE SIMILARITY** để tính content score (KHÔNG phải Euclidean Distance)
- Phần dưới đây dùng **Euclidean Distance để MINH HỌA** vấn đề normalization dễ hiểu
- Nhưng **cả Cosine Similarity và Euclidean đều bị ảnh hưởng** nếu không normalize!
