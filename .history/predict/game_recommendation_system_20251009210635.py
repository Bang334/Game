...
class GameRecommendationSystem:
    ...
    def create_temporal_impact_chart(self, user_id, keyword="", filename=None):
        ...
        # === SUBPLOT 2: Boost Factor Breakdown (Stacked Bars) ===
        # Extract boost factor data
        factor_names = ['Publisher', 'Genre', 'Price', 'Age', 'Mode', 'Platform']
        factor_keys = ['publisher', 'genre', 'price', 'age_rating', 'mode', 'platform']
        
        # Build data for stacked bars (convert multipliers to percentage contribution)
        ...
        for game_id, data in sorted_games:
            ...
            if not boost_factors:
                # No boost data - fill with 1.0
                for name in factor_names:
                    factor_data[name].append(0)
            else:
                # Convert multipliers to additive percentages
                # e.g., 1.5 -> +50%, 1.2 -> +20%, 1.0 -> 0%
                for name, key in zip(factor_names, factor_keys):
                    multiplier = boost_factors.get(key, 1.0)
                    percentage = (multiplier - 1.0) * 100  # Convert to percentage
        ...
        ax2.set_xlabel('Boost Percentage (%)', fontsize=11, fontweight='bold')
        ax2.set_title('Boost Factor Breakdown\n(7-Day Preferences)', fontsize=12, fontweight='bold')
        ax2.set_yticks(x)
        ...
        for i, (game_id, data) in enumerate(sorted_games):
            boost_factors = data.get('boost_factors', {})
            if boost_factors:
                total_multiplier = boost_factors.get('total', 1.0)
                ax2.text(left_offset[i] + 2, i, f'×{total_multiplier:.2f}',
