...
class GameRecommendationSystem:
    ...
    def create_temporal_impact_chart(self, user_id, keyword="", filename=None):
        ...
        So sánh scores của TẤT CẢ games (trừ games đã tương tác) với breakdown chi tiết
        """
        if not MATPLOTLIB_AVAILABLE:
        ...
        # Lấy danh sách games user đã tương tác (để loại bỏ)
        target_user = next((u for u in self.users_data if u['id'] == user_id), None)
        if not target_user:
        ...
        # Games đã mua
        interacted_game_ids.update(target_user.get('purchased_games', {}).keys())
        # Games đã favorite
        interacted_game_ids.update(target_user.get('favorite_games', []))
        # Games đã view (nếu có)
        if 'viewed_games' in target_user:
            interacted_game_ids.update(target_user.get('viewed_games', []))
        
        print(f"🚫 Excluding {len(interacted_game_ids)} games user already interacted with")
        ...
        recs_base = self.get_hybrid_recommendations(
            user_id=user_id,
            top_n=1000,  # Lấy nhiều để có đủ sau khi filter
            keyword=keyword,
        ...
        # Filter out interacted games
        recs_base = [r for r in recs_base if r['game_id'] not in interacted_game_ids]
        recs_7days = [r for r in recs_7days if r['game_id'] not in interacted_game_ids]
        
        print(f"📊 Analyzing {len(recs_base)} games after filtering...")
        ...
        # Add base scores (NO boost) - TOÀN BỘ
        for rec in recs_base:  # TẤT CẢ games
            game_id = rec['game_id']
        ...
        # Add 7-day boosted scores - TOÀN BỘ
        for rec in recs_7days:  # TẤT CẢ games
            game_id = rec['game_id']
        ...
        # Sort by absolute impact - hiển thị TẤT CẢ (hoặc giới hạn hợp lý)
        sorted_games = sorted(game_scores.items(), key=lambda x: abs(x[1]['impact']), reverse=True)
        
        # Limit để chart không quá dài (có thể điều chỉnh)
        max_games_display = min(len(sorted_games), 50)  # Tối đa 50 games
        sorted_games = sorted_games[:max_games_display]
        
        print(f"📊 Displaying {len(sorted_games)} games in chart (sorted by impact)")
        
        # Create figure with 3 subplots (dynamic height based on number of games)
        height = max(12, len(sorted_games) * 0.4)  # 0.4 inch per game, min 12
