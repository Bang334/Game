...
class GameRecommendationSystem:
    ...
    def create_temporal_impact_chart(self, user_id, keyword="", filename=None):
        ...
        # Print summary with boost factor comparison
        print("\n" + "="*90)
        print("📊 TEMPORAL IMPACT SUMMARY WITH BOOST FACTOR COMPARISON")
        print("="*90)
        ...
        for i, (game_id, data) in enumerate(boosted, 1):
            if data['impact'] > 0:
                ...
                if total_7days > total_alltime:
                    print(f"     Boost: All-time ×{total_alltime:.2f} → 7-day ×{total_7days:.2f} ↑ (stronger match)")
                else:
                    print(f"     Boost: All-time ×{total_alltime:.2f} → 7-day ×{total_7days:.2f} (still boosted)")
        ...
        for i, (game_id, data) in enumerate(weakened, 1):
            if data['impact'] < 0:
                ...
                # Show why weakened - boost factor comparison
                bf_alltime = data.get('boost_factors_alltime', {})
                bf_7days = data.get('boost_factors_7days', {})
                ...
                print(f"     Boost: All-time ×{total_alltime:.2f} → 7-day ×{total_7days:.2f} ↓ (weaker match now)")
        
        print("\n💡 Explanation: Games weakened because their boost factor DECREASED")
        print("   (All-time boost > 7-day boost = user's preferences shifted away)")
        print("="*90)
