...
class GameRecommendationSystem:
    ...
    def create_temporal_impact_chart(self, user_id, keyword="", filename=None):
        ...
        # User info and title
        user_data = next((u for u in self.users_data if u['id'] == user_id), None)
        fig.suptitle(f'Temporal Impact Analysis - {user_data["name"] if user_data else f"User {user_id}"}\n'
                    f'Score Changes & Boost Factor Evolution Over Time',
                    fontsize=14, fontweight='bold', y=0.98)
        
        # Add explanation
        explanation = (
            f"Left: Final scores (Blue=All-time, Red=7 days) | "
            f"Middle: Boost factors (Blue=All-time, Red=7 days) - Shows WHY scores changed | "
            f"Right: Net impact (🟢=Score increased, 🔴=Score decreased)"
        )
