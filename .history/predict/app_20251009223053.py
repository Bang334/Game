...
def api_recommendations(user_id):
    """
    API lấy recommendations - TRẢ JSON TRỰC TIẾP
    
    Query params:
    - keyword: từ khóa tìm kiếm (optional)
    - top_n: số lượng recommendations (default: 10)
    ...
    # Get recommendations
    recommendations = get_smart_recommendations(user_id, top_n=top_n, keyword=keyword)
    
    # Format response với đầy đủ thông tin game
    games_list = []
    for rec in recommendations:
        # Tìm thông tin đầy đủ của game
        ...
        if game_full_info:
            game_info = {
                "id": rec['game_id'],
                "name": rec['game_name'],
                "description": game_full_info.get('description', ''),
                "rating": rec.get('actual_rating', 0),
                "release_date": game_full_info.get('release_date', '2020-01-01'),
                "publisher": game_full_info.get('publisher', ''),
                "genre": rec.get('genre', []),
                "mode": game_full_info.get('mode', ''),
                "price": rec.get('price', 0),
                "downloads": rec.get('downloads', 0),
                "min_spec": {k: v for k, v in (game_full_info.get('min_spec', {}) or {}).items() if k != 'storage'},
                "rec_spec": {k: v for k, v in (game_full_info.get('rec_spec', {}) or {}).items() if k != 'storage'},
                "multiplayer": game_full_info.get('multiplayer', False),
                "capacity": game_full_info.get('capacity', 0),
                "age_rating": game_full_info.get('age_rating', ''),
                "platform": game_full_info.get('platform', []),
                "language": game_full_info.get('language', []),
                "link_download": rec.get('link_download', ''),
                "image": game_full_info.get('image', ''),
                "score": rec.get('hybrid_score', 0),  # AI recommendation score (đã boost)
                "boost_factor": rec.get('boost_factor', 1.0),  # Hệ số boost
