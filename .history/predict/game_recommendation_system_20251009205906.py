...
class GameRecommendationSystem:
    ...
    def create_temporal_impact_chart(self, user_id, keyword="", filename=None):
        ...
        game_names = [data['name'][:20] for _, data in sorted_games]
        ...
        ax1.set_xlabel('Hybrid Score', fontsize=12, fontweight='bold')
        ax1.set_ylabel('Games', fontsize=12, fontweight='bold')
        ax1.set_title('Score Comparison: All-time vs Recent 7 Days', fontsize=14, fontweight='bold')
        ax1.set_yticks(x)
        ax1.set_yticklabels(game_names, fontsize=9)
        ...
        for i, (bar1, bar2) in enumerate(zip(bars1, bars2)):
            if alltime_scores[i] > 0:
                ax1.text(alltime_scores[i] + 0.01, bar1.get_y() + bar1.get_height()/2, 
                        f'{alltime_scores[i]:.3f}', va='center', fontsize=8)
            if days7_scores[i] > 0:
                ax1.text(days7_scores[i] + 0.01, bar2.get_y() + bar2.get_height()/2,
                        f'{days7_scores[i]:.3f}', va='center', fontsize=8)
        
        # === SUBPLOT 2: Impact Analysis (Score Change) ===
