...
class GameRecommendationSystem:
    ...
    def create_temporal_impact_chart(self, user_id, keyword="", filename=None):
        ...
        bars3 = ax2.barh(x, impacts, color=colors, alpha=0.8)
        
        ax2.set_xlabel('Score Change (7 Days - All-time)', fontsize=12, fontweight='bold')
        ax2.set_ylabel('Games', fontsize=12, fontweight='bold')
        ax2.set_title('Impact of Recent Interactions', fontsize=14, fontweight='bold')
        ax2.set_yticks(x)
        ax2.set_yticklabels(game_names, fontsize=9)
        ax2.axvline(x=0, color='black', linestyle='-', linewidth=0.8)
        ax2.grid(True, alpha=0.3, axis='x')
        ax2.set_axisbelow(True)
        
        # Add impact values and arrows
        for i, (bar, impact) in enumerate(zip(bars3, impacts)):
            x_pos = impact + (0.02 if impact > 0 else -0.02)
            ...
            if abs(impact) > 0.05:
                ...
                ax2.text(0, bar.get_y() + bar.get_height()/2, arrow,
        ...
        fig.suptitle(f'Temporal Impact Analysis - {user_data["name"] if user_data else f"User {user_id}"}\n'
                    f'How Recent Interactions (7 Days) Affect Game Scores',
                    fontsize=16, fontweight='bold', y=0.98)
        
        # Add explanation
        explanation = (
            f"🟢 Green bars: Games boosted by recent interactions\n"
            f"🔴 Red bars: Games weakened by recent preferences shift"
        )
