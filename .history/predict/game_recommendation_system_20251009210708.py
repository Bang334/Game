...
class GameRecommendationSystem:
    ...
    def _calculate_boost_factor_breakdown(self, game, user_preferences):
        ...
        Returns dict với các components: publisher, genre, price, age_rating, mode, platform
        """
        breakdown = {
            'publisher': 1.0,
            'genre': 1.0,
        ...
        # Publisher boost
        publisher = game.get('publisher', '')
        if publisher and publisher in user_preferences.get('publishers', {}):
        ...
        genre_boosts = []
        for genre in genres:
            if genre in user_preferences.get('genres', {}):
        ...
        if genre_boosts:
            breakdown['genre'] = max(genre_boosts)
        
        # Price boost
        price = game.get('price', 0)
        price_prefs = user_preferences.get('price', {})
        if price_prefs and price > 0:
            avg_price = price_prefs.get('avg', 0)
            std_price = price_prefs.get('std', 0)
            
            if std_price > 0:
                if abs(price - avg_price) <= std_price:
                    breakdown['price'] = 1.2
                elif abs(price - avg_price) <= 2 * std_price:
                    breakdown['price'] = 1.1
        
        # Age rating boost
        age_rating = game.get('age_rating', '')
        if age_rating in user_preferences.get('age_ratings', {}):
        ...
        # Mode boost
        mode = game.get('mode', '')
        if mode in user_preferences.get('modes', {}):
        ...
        for platform in platforms:
            if platform in user_preferences.get('platforms', {}):
        ...
        if platform_boosts:
            breakdown['platform'] = max(platform_boosts)
    ...
    def create_temporal_impact_chart(self, user_id, keyword="", filename=None):
        ...
        # Get recommendations - All time
        print("📊 Calculating all-time recommendations...")
        ...
        # Get recommendations - 7 days
        print("📊 Calculating 7-day recommendations...")
        ...
        # Get user preferences for both periods to extract boost factors
        prefs_alltime = self.analyze_user_preferences(user_id, recent_days=None)
        prefs_7days = self.analyze_user_preferences(user_id, recent_days=7)
        
        # Create score comparison with boost factor details
        game_scores = {}
        
        # Add all-time scores with boost factors
        for rec in recs_alltime[:20]:  # Top 20
            game_id = rec['game_id']
            
            # Get game details and calculate all-time boost
            game = next((g for g in self.games_data if g['id'] == game_id), None)
            boost_alltime = self._calculate_boost_factor_breakdown(game, prefs_alltime) if game else {}
            
            game_scores[game_id] = {
                'name': rec['game_name'],
                'alltime_score': rec['hybrid_score'],
                '7days_score': 0,
                'alltime_rank': recs_alltime.index(rec) + 1,
                '7days_rank': None,
                'boost_factors_alltime': boost_alltime,
                'boost_factors_7days': {}  # Will be filled later
            }
        
        # Add 7-day scores and calculate boost factors
        for rec in recs_7days[:20]:
            game_id = rec['game_id']
            ...
            boost_7days = self._calculate_boost_factor_breakdown(game, prefs_7days)
            boost_alltime = self._calculate_boost_factor_breakdown(game, prefs_alltime)
            
            if game_id in game_scores:
                game_scores[game_id]['7days_score'] = rec['hybrid_score']
                game_scores[game_id]['7days_rank'] = recs_7days.index(rec) + 1
                game_scores[game_id]['boost_factors_7days'] = boost_7days
                # All-time boost already filled
            else:
                # Game appeared in 7-day but not in all-time top 20
                game_scores[game_id] = {
                    'name': rec['game_name'],
                    'alltime_score': 0,
                    '7days_score': rec['hybrid_score'],
                    'alltime_rank': None,
                    '7days_rank': recs_7days.index(rec) + 1,
                    'boost_factors_alltime': boost_alltime,
                    'boost_factors_7days': boost_7days
                }
        
        # Calculate impact (score difference)
        for game_id, data in game_scores.items():
            data['impact'] = data['7days_score'] - data['alltime_score']
            data['impact_percent'] = (data['impact'] / data['alltime_score'] * 100) if data['alltime_score'] > 0 else 0
        
        # Sort by absolute impact
        sorted_games = sorted(game_scores.items(), key=lambda x: abs(x[1]['impact']), reverse=True)[:12]
        ...
        alltime_scores = [data['alltime_score'] for _, data in sorted_games]
        days7_scores = [data['7days_score'] for _, data in sorted_games]
        
        x = np.arange(len(game_names))
        width = 0.35
        
        bars1 = ax1.barh(x + width/2, alltime_scores, width, label='All-time', color='#3498db', alpha=0.8)
        bars2 = ax1.barh(x - width/2, days7_scores, width, label='7 Days', color='#e74c3c', alpha=0.8)
        
        ax1.set_xlabel('Hybrid Score', fontsize=11, fontweight='bold')
        ax1.set_ylabel('Games', fontsize=11, fontweight='bold')
        ax1.set_title('Score Comparison\nAll-time vs 7 Days', fontsize=12, fontweight='bold')
        ax1.set_yticks(x)
        ...
        for i, (bar1, bar2) in enumerate(zip(bars1, bars2)):
            if alltime_scores[i] > 0:
                ax1.text(alltime_scores[i] + 0.01, bar1.get_y() + bar1.get_height()/2, 
                        f'{alltime_scores[i]:.2f}', va='center', fontsize=7)
            if days7_scores[i] > 0:
                ax1.text(days7_scores[i] + 0.01, bar2.get_y() + bar2.get_height()/2,
                        f'{days7_scores[i]:.2f}', va='center', fontsize=7)
        
        # === SUBPLOT 2: Boost Factor Comparison (All-time vs 7-day) ===
        # Extract boost multipliers for comparison
        boost_alltime_totals = []
        boost_7days_totals = []
        boost_changes = []
        
        for game_id, data in sorted_games:
            bf_alltime = data.get('boost_factors_alltime', {})
            bf_7days = data.get('boost_factors_7days', {})
            
            total_alltime = bf_alltime.get('total', 1.0)
            total_7days = bf_7days.get('total', 1.0)
            
            boost_alltime_totals.append(total_alltime)
            boost_7days_totals.append(total_7days)
            boost_changes.append(total_7days - total_alltime)
        
        # Create grouped bars for boost comparison
        width = 0.35
        
        bars_alltime = ax2.barh(x + width/2, boost_alltime_totals, width, 
                               label='All-time Boost', color='#3498db', alpha=0.8)
        bars_7days = ax2.barh(x - width/2, boost_7days_totals, width,
                             label='7-Day Boost', color='#e74c3c', alpha=0.8)
        
        ax2.set_xlabel('Total Boost Multiplier', fontsize=11, fontweight='bold')
        ax2.set_title('Boost Factor Comparison\nAll-time vs 7-Day', fontsize=12, fontweight='bold')
        ax2.set_yticks(x)
        ax2.set_yticklabels(game_names, fontsize=8)
        ax2.legend(loc='lower right', fontsize=9)
        ax2.grid(True, alpha=0.3, axis='x')
        ax2.set_axisbelow(True)
        ax2.axvline(x=1.0, color='black', linestyle='--', linewidth=0.5, alpha=0.5)
        
        # Add boost multiplier values
        for i, (bar_alltime, bar_7days) in enumerate(zip(bars_alltime, bars_7days)):
            # All-time value
            if boost_alltime_totals[i] > 0:
                ax2.text(boost_alltime_totals[i] + 0.05, bar_alltime.get_y() + bar_alltime.get_height()/2,
                        f'×{boost_alltime_totals[i]:.2f}', va='center', fontsize=7)
            
            # 7-day value
            if boost_7days_totals[i] > 0:
                ax2.text(boost_7days_totals[i] + 0.05, bar_7days.get_y() + bar_7days.get_height()/2,
                        f'×{boost_7days_totals[i]:.2f}', va='center', fontsize=7)
            
            # Show change arrow if significant
            change = boost_changes[i]
            if abs(change) >= 0.2:
                arrow = '↑' if change > 0 else '↓'
                arrow_color = '#27ae60' if change > 0 else '#e74c3c'
                ax2.text(max(boost_alltime_totals[i], boost_7days_totals[i]) + 0.15, i,
                        arrow, fontsize=10, fontweight='bold', color=arrow_color)
        ...
        # User info and title
        user_data = next((u for u in self.users_data if u['id'] == user_id), None)
        fig.suptitle(f'Temporal Impact Analysis with Boost Factor Breakdown - {user_data["name"] if user_data else f"User {user_id}"}\n'
                    f'How Recent Interactions (7 Days) & Preference Factors Affect Game Scores',
                    fontsize=14, fontweight='bold', y=0.98)
        
        # Add explanation
        explanation = (
            f"Left: Score comparison (Blue=All-time, Red=7 days) | "
            f"Middle: Boost factor breakdown (Publisher, Genre, Price, etc.) | "
            f"Right: Net impact (🟢=Boosted, 🔴=Weakened)"
        )
        ...
        print("\n" + "="*80)
        print("📊 TEMPORAL IMPACT SUMMARY WITH BOOST FACTORS")
        print("="*80)
        ...
        for i, (game_id, data) in enumerate(boosted, 1):
            if data['impact'] > 0:
                ...
                print(f"     Score: {data['alltime_score']:.3f} → {data['7days_score']:.3f} ({data['impact']:+.3f})")
                
                # Show boost factor breakdown
                bf = data.get('boost_factors', {})
                if bf and bf.get('total', 1.0) > 1.0:
                    boost_parts = []
                    if bf.get('publisher', 1.0) > 1.0:
                        boost_parts.append(f"Publisher ×{bf['publisher']:.2f}")
                    if bf.get('genre', 1.0) > 1.0:
                        boost_parts.append(f"Genre ×{bf['genre']:.2f}")
                    if bf.get('price', 1.0) > 1.0:
                        boost_parts.append(f"Price ×{bf['price']:.2f}")
                    if bf.get('age_rating', 1.0) > 1.0:
                        boost_parts.append(f"Age ×{bf['age_rating']:.2f}")
                    if bf.get('mode', 1.0) > 1.0:
                        boost_parts.append(f"Mode ×{bf['mode']:.2f}")
                    if bf.get('platform', 1.0) > 1.0:
                        boost_parts.append(f"Platform ×{bf['platform']:.2f}")
                    
                    if boost_parts:
                        print(f"     Boost Factors: {' | '.join(boost_parts)}")
                        print(f"     Total Boost: ×{bf['total']:.2f}")
        
        print(f"\nTop 5 Games WEAKENED by Preference Shift:")
        weakened = sorted(sorted_games, key=lambda x: x[1]['impact'])[:5]
        for i, (game_id, data) in enumerate(weakened, 1):
            if data['impact'] < 0:
                print(f"  {i}. {data['name'][:40]}")
                print(f"     Score: {data['alltime_score']:.3f} → {data['7days_score']:.3f} ({data['impact']:+.3f})")
                
                # Show why weakened (lack of boost factors)
                bf = data.get('boost_factors', {})
                if bf:
                    print(f"     Total Boost: ×{bf.get('total', 1.0):.2f} (low preference match)")
