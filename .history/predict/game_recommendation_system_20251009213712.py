...
class GameRecommendationSystem:
    ...
    def _calculate_boost_factor_breakdown(self, game, user_preferences):
        ...
        if genres and genres_pref:
            ...
            for genre in genres:
                if genre in genres_pref:
                    matched = True
                    score = genres_pref[genre]
                    if score >= 0.5:  # Very strong match
                        genre_boosts.append(1.2)
                    elif score >= 0.3:  # Strong match
                        genre_boosts.append(1.15)
                    elif score >= 0.15:  # Medium match
                        genre_boosts.append(1.1)
                    else:  # Weak match
                        genre_boosts.append(1.05)
            
            if genre_boosts:
                breakdown['genre'] = max(genre_boosts)
            elif not matched:
                # Không có genre nào match
                breakdown['genre'] = 0.85
        
        # Price boost (0.9 - 1.15)
        price = game.get('price', 0)
        ...
        if price_prefs and price > 0:
            ...
            if std_price > 0 and avg_price > 0:
                diff = abs(price - avg_price)
                if diff <= std_price:  # Within 1 std
                    breakdown['price'] = 1.15
                elif diff <= 1.5 * std_price:  # Within 1.5 std
                    breakdown['price'] = 1.1
                elif diff <= 2 * std_price:  # Within 2 std
                    breakdown['price'] = 1.05
                else:  # Outside price range
                    breakdown['price'] = 0.95
        
        # Age rating boost (0.95 - 1.15)
        age_rating = game.get('age_rating', '')
        ...
        if mode and modes_pref:
            if mode in modes_pref:
                ...
                if score >= 0.5:  # Strong preference
                    breakdown['mode'] = 1.15
                elif score >= 0.3:  # Medium preference
                    breakdown['mode'] = 1.1
                else:  # Weak preference
                    breakdown['mode'] = 1.05
            else:
    ...
    def create_temporal_impact_chart(self, user_id, keyword="", filename=None):
        ...
        # Sort by absolute impact - show top 20 games
        sorted_games = sorted(game_scores.items(), key=lambda x: abs(x[1]['impact']), reverse=True)[:20]
        
        # Create figure with 3 subplots (tăng height cho 20 games)
