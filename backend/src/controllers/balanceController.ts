...
export class BalanceController {
   ...
   */
  static async getUserPendingDeposits(req: Request, res: Response) {
    try {
      const userId = (req as any).user?.user_id
   ...
   */
  static async getAllDepositRequests(req: Request, res: Response) {
    try {
      const { 
        status, 
        start_date, 
      ...
      const deposits = (rows as any[]).map(row => ({
  ...
  static async approveDeposit(req: Request, res: Response) {
    try {
      ...
      await pool.execute(
        ...
        [adminUserId, newBalance, transactionId]
      )
