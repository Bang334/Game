 ...
 */
router.get('/interactions/:userId', async (req: Request, res: Response) => {
  try {
...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    db.all(statsQuery, [], (err, typeStats: any[]) => {
      if (err) {
      ...
      db.get(countsQuery, [], (err, counts: any) => {
        db.close();
...
router.get('/behavior/:userId', async (req: Request, res: Response) => {
  try {
    ...
    db.get(behaviorQuery, [userId], (err, behavior: any) => {
      if (err) {
      ...
      db.all(recentInteractionsQuery, [userId], (err, recentStats: any[]) => {
        db.close();
...
router.get('/analytics', async (req: Request, res: Response) => {
  try {
    ...
    db.all(analyticsQuery, [], (err, metrics: any[]) => {
      if (err) {
      ...
      db.all(trendsQuery, [], (err, trends: any[]) => {
        db.close();
 ...
 */
router.delete('/interactions/:userId', async (req: Request, res: Response) => {
  try {
    const userId = parseInt(req.params.userId || '0');
