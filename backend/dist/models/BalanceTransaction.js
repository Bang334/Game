"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.BalanceTransactionModel = void 0;
const db_1 = require("../db");
exports.BalanceTransactionModel = {
    async create(data) {
        const [result] = await db_1.pool.execute(`INSERT INTO balance_transaction 
       (user_id, amount, balance_before, balance_after, transaction_type, status, description, related_game_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
            data.user_id,
            data.amount,
            data.balance_before,
            data.balance_after,
            data.transaction_type,
            data.status || 'PENDING',
            data.description || null,
            data.related_game_id || null
        ]);
        return result.insertId;
    },
    async findByUserId(userId, limit = 50) {
        const [rows] = await db_1.pool.execute(`SELECT 
        bt.*,
        g.name as game_name,
        g.image as game_image,
        u.username as reviewer_name
       FROM balance_transaction bt
       LEFT JOIN game g ON bt.related_game_id = g.game_id
       LEFT JOIN user u ON bt.reviewed_by = u.user_id
       WHERE bt.user_id = ? AND bt.status = 'APPROVED'
       ORDER BY bt.created_at DESC
       LIMIT ${Math.max(1, Math.min(limit, 1000))}`, [userId]);
        return rows;
    },
    async findById(transactionId) {
        const [rows] = await db_1.pool.execute(`SELECT * FROM balance_transaction WHERE transaction_id = ? LIMIT 1`, [transactionId]);
        return rows.length > 0 ? rows[0] : null;
    },
    async getStatsByUserId(userId) {
        const [rows] = await db_1.pool.execute(`SELECT 
        SUM(CASE WHEN transaction_type = 'DEPOSIT' AND status = 'APPROVED' THEN amount ELSE 0 END) as total_deposits,
        SUM(CASE WHEN transaction_type = 'PURCHASE' AND status = 'APPROVED' THEN ABS(amount) ELSE 0 END) as total_purchases,
        COUNT(*) as transaction_count
       FROM balance_transaction
       WHERE user_id = ? AND status = 'APPROVED'`, [userId]);
        return rows[0];
    },
    async findPendingDeposits(limit = 50) {
        const [rows] = await db_1.pool.execute(`SELECT 
        bt.*,
        u.username,
        u.email as user_email
       FROM balance_transaction bt
       JOIN user u ON bt.user_id = u.user_id
       WHERE bt.transaction_type = 'DEPOSIT' AND bt.status = 'PENDING'
       ORDER BY bt.created_at ASC
       LIMIT ${Math.max(1, Math.min(limit, 1000))}`, []);
        return rows;
    },
    async approveDeposit(transactionId, adminUserId) {
        const [result] = await db_1.pool.execute(`UPDATE balance_transaction 
       SET status = 'APPROVED', reviewed_by = ?, reviewed_at = NOW()
       WHERE transaction_id = ? AND transaction_type = 'DEPOSIT' AND status = 'PENDING'`, [adminUserId, transactionId]);
        return result.affectedRows > 0;
    },
    async rejectDeposit(transactionId, adminUserId, reason) {
        const [result] = await db_1.pool.execute(`UPDATE balance_transaction 
       SET status = 'REJECTED', reviewed_by = ?, reviewed_at = NOW(), 
           description = CONCAT(COALESCE(description, ''), ' - Từ chối: ', ?)
       WHERE transaction_id = ? AND transaction_type = 'DEPOSIT' AND status = 'PENDING'`, [adminUserId, reason || 'Không có lý do', transactionId]);
        return result.affectedRows > 0;
    },
    async getUserPendingDeposits(userId) {
        const [rows] = await db_1.pool.execute(`SELECT bt.*
       FROM balance_transaction bt
       WHERE bt.user_id = ? AND bt.transaction_type = 'DEPOSIT' AND bt.status = 'PENDING'
       ORDER BY bt.created_at DESC`, [userId]);
        return rows;
    }
};
//# sourceMappingURL=BalanceTransaction.js.map