...
class RecommendationController {
    ...
    static async getRecommendations(req, res) {
        try {
            ...
            const recentDays = days ? parseInt(days) : null; // null = all time (SQLite data is old)
            ...
            try {
                ...
                const aiResponse = await axios_1.default.post(`${AI_SERVICE_URL}/api/recommend`, requestBody, {
                ...
                const recommendations = (aiResponse.data.games || []).map((game) => ({
                    game_id: game.id,
                    name: game.name,
                    price: game.price,
                    image: game.image || '',
     ...
     */
    static async checkAIServiceHealth(req, res) {
        try {
            const healthResponse = await axios_1.default.get(`${AI_SERVICE_URL}/health`, {
                timeout: 5000
            });
            res.json({
                success: true,
