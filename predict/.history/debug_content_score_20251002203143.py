...
def debug_content_score():
    ...
    for price in prices:
        ...
        if red_dead_idx is not None:
            ...
            # Tinh Content Score cho User 1 voi Red Dead
            content_recs = recommender.get_content_recommendations(user_id=1, top_n=10)
            red_dead_content_score = None
            for game_id, score in content_recs:
                if game_id == 7:  # Red Dead
                    red_dead_content_score = score
                    break
            
            print(f"Red Dead Content Score for User 1: {red_dead_content_score:.4f}")
