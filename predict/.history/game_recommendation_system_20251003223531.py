...
class GameRecommendationSystem:
    ...
    def build_content_similarity(self):
        ...
        try:
            ...
            # Chuẩn hóa similarity matrix về 0-1 với scaling tốt hơn
            # Thay vì (similarity + 1) / 2, sử dụng min-max scaling
            min_sim = similarity_matrix.min()
            max_sim = similarity_matrix.max()
            
            if max_sim > min_sim:
                # Min-max scaling để có range 0-1
                self.content_similarity_matrix = (similarity_matrix - min_sim) / (max_sim - min_sim)
            else:
                # Nếu tất cả giống nhau, set về 0.5
                self.content_similarity_matrix = np.full_like(similarity_matrix, 0.5)
            
            print(f"Normalized similarity range: {self.content_similarity_matrix.min():.3f} - {self.content_similarity_matrix.max():.3f}")
            print(f"Normalized similarity mean: {self.content_similarity_matrix.mean():.3f}")
