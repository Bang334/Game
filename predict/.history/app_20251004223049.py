        ...
        if recommender:
            try:
                ...
                if hasattr(recommender, 'user_item_matrix') and recommender.user_item_matrix is not None:
                    ...
                    # Mean rating chỉ tính nếu có dữ liệu
                    if non_zero_count > 0:
                        non_zero_values = matrix[matrix != 0]
                        retrain_info['model_parameters']['user_item_matrix']['mean_rating'] = float(non_zero_values.mean().item())
                        retrain_info['model_parameters']['user_item_matrix']['min_rating'] = float(non_zero_values.min().item())
                        retrain_info['model_parameters']['user_item_matrix']['max_rating'] = float(non_zero_values.max().item())
