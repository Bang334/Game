...
class GameRecommendationSystem:
    ...
    def build_content_similarity(self):
        """Xây dựng ma trận tương đồng content-based tự nhiên"""
        try:
            from sklearn.preprocessing import StandardScaler
            import numpy as np
            ...
            for game in self.games_data:
                ...
                # Tạo genre string tự nhiên (không lặp lại)
                ...
                # Kết hợp tất cả features một cách tự nhiên
                combined_feature = f"{genre_str} {rating_str} {price_range} {platform_str} {mode} {multiplayer}"
            ...
            # Tính cosine similarity tự nhiên (không thêm baseline)
            similarity_matrix = cosine_similarity(feature_matrix)
            
            # Lưu ma trận similarity gốc
            self.content_similarity_matrix = similarity_matrix
            
            print("Xay dung content similarity matrix tu nhien thanh cong!")
            print(f"Features: {feature_matrix.shape[1]} dimensions")
            print("Content similarity calculated naturally without baseline")
