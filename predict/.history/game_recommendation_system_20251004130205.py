...
class GameRecommendationSystem:
    ...
    def build_content_similarity(self):
        """Xây dựng ma trận tương đồng content-based đơn giản và cơ bản"""
        try:
            from sklearn.preprocessing import StandardScaler
            import numpy as np
            
            # Tạo features đơn giản chỉ dựa trên genre và rating
            features = []
            
            for game in self.games_data:
                # === GENRE FEATURES (quan trọng nhất) ===
                ...
                rating_str = f"rating_{int(rating)}" if rating > 0 else "rating_0"
                ...
                combined_feature = f"{genre_weighted} {rating_str} {price_range} {platform_str} {mode} {multiplayer}"
                features.append(combined_feature)
            
            # Tính TF-IDF cho features
            ...
            # Đảm bảo tất cả games đều có similarity score > 0
            # Thêm một baseline similarity nhỏ để tránh score = 0
            baseline_similarity = 0.15  # Tăng baseline lên 0.15
            similarity_matrix = np.maximum(similarity_matrix, baseline_similarity)
            
            # Chuẩn hóa về 0-1 range
            self.content_similarity_matrix = np.clip(similarity_matrix, 0, 1)
            
            print("Xay dung content similarity matrix don gian thanh cong!")
            print(f"Features: {feature_matrix.shape[1]} dimensions")
            print("Content similarity normalized to 0-1 range with baseline 0.1")
            print("All games now have minimum content score > 0")
            return True
        except Exception as e:
    ...
    def get_hybrid_recommendations(self, user_id, top_n=10, keyword=""):
        ...
        for rec in content_recs:
            ...
            if game_id in all_games:
                all_games[game_id]['content_score'] = max(rec['similarity_score'], 0.1)
            else:
                all_games[game_id] = {
                    'game_id': game_id,
                    'game_name': rec['game_name'],
                    'svd_score': 0,
                    'content_score': max(rec['similarity_score'], 0.1),
                    'demographic_score': 0,
                    'keyword_score': 0,
                    'actual_rating': rec['actual_rating'],
                    'genre': rec['genre'],
        ...
        # Thêm tất cả games để tính keyword score và đảm bảo content score > 0
        for game in self.games_data:
            game_id = game['id']
            if game_id not in all_games:
                # Tính content score cho game này
                content_score = 0.1  # Baseline content score
                if self.content_similarity_matrix is not None:
                    # Tính content score dựa trên similarity với tất cả games khác
                    game_idx = game_id - 1
                    if 0 <= game_idx < len(self.content_similarity_matrix):
                        # Lấy similarity với tất cả games khác và tính trung bình
                        similarities = self.content_similarity_matrix[game_idx]
                        # Loại bỏ similarity với chính nó (diagonal)
                        other_similarities = [sim for i, sim in enumerate(similarities) if i != game_idx]
                        if other_similarities:
                            content_score = max(np.mean(other_similarities), 0.1)
        ...
        for game_id in all_games:
            if all_games[game_id]['content_score'] == 0:
                # Tính content score cho game này nếu chưa có
                content_score = 0.1  # Baseline content score
                if self.content_similarity_matrix is not None:
                    game_idx = game_id - 1
                    if 0 <= game_idx < len(self.content_similarity_matrix):
