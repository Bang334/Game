...
class GameRecommendationSystem:
    ...
    def build_content_similarity(self):
        ...
        try:
            ...
            for game in self.games_data:
                ...
                # Downloads - trọng số cao hơn vì quan trọng cho recommendation
                downloads = game.get('downloads', 0)
                # Chuẩn hóa downloads về log scale và tăng trọng số
                downloads_normalized = np.log10(max(downloads, 1)) * 0.8  # Trọng số cao hơn technical specs
            ...
            for game in self.games_data:
                genre_list = game.get('genre', [])
                # Tạo genre string với trọng số cao (lặp lại 8 lần để tăng similarity)
                genre_weighted = ' '.join([genre for genre in genre_list for _ in range(8)])
            ...
            # Kết hợp: Genre (5x weight) + Text + Numeric (0.3x weight) + Downloads (0.8x weight)
            from scipy.sparse import hstack, csr_matrix
            numeric_sparse = csr_matrix(numeric_matrix)
            # Kết hợp với trọng số: Genre (5x), Text (1x), Numeric (0.3x), Downloads (0.8x)
            combined_matrix = hstack([genre_matrix, text_matrix, numeric_sparse])
            
            # Tính cosine similarity
            similarity_matrix = cosine_similarity(combined_matrix)
            
            # Chuẩn hóa và boost similarity matrix để tăng content scores
            # Cosine similarity thường từ 0 đến 1, boost lên để có scores cao hơn
            similarity_boosted = similarity_matrix * 1.5  # Boost 50%
            
            # Thêm baseline similarity để tránh scores = 0 hoàn toàn
            # Mỗi game sẽ có ít nhất 0.1 similarity với games khác
            baseline_similarity = 0.1
            similarity_with_baseline = np.maximum(similarity_boosted, baseline_similarity)
            
            self.content_similarity_matrix = np.clip(similarity_with_baseline, 0, 1)
            
            print("Xay dung content similarity matrix voi nhieu thuoc tinh thanh cong!")
            print(f"Genre features: {genre_matrix.shape[1]} dimensions (8x weight)")
            print(f"Text features: {text_matrix.shape[1]} dimensions (1x weight)")
            print(f"Numeric features: {numeric_matrix.shape[1]} dimensions (0.3x weight, 0.8x for downloads)") 
            print(f"Total features: {combined_matrix.shape[1]} dimensions")
            print("Content similarity normalized to 0-1 range with 0.1 baseline")
            print("Genre similarity now has 8x higher weight than technical specs!")
            print("Downloads field added with 0.8x weight (higher than other technical specs)!")
            return True
        except Exception as e:
    ...
    def get_content_recommendations(self, user_id, top_n=20):
    ...
    def get_demographic_recommendations(self, user_id, top_n=20):
