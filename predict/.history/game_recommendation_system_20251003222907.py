...
class GameRecommendationSystem:
    ...
    def optimize_content_weights(self, test_users=None, metric='diversity'):
        """
        Tự động tối ưu trọng số cho content similarity
        """
        if test_users is None:
            # Lấy 5 users ngẫu nhiên để test
            test_users = np.random.choice([u['id'] for u in self.users_data], 
                                        min(5, len(self.users_data)), replace=False)
        
        print(f"Tối ưu trọng số cho {len(test_users)} users...")
        
        # Định nghĩa các trọng số cần test
        genre_weights = [2, 3, 4, 5]
        text_weights = [1, 1.5, 2, 2.5]
        numeric_weights = [0.5, 1, 1.5, 2]
        ...
        # Grid search
        for genre_w, text_w, numeric_w in product(genre_weights, text_weights, numeric_weights):
            try:
                # Tạo content similarity matrix với trọng số mới
                temp_similarity = self._build_content_similarity_with_weights(
                    genre_w, text_w, numeric_w
                )
                
                # Test performance
                score = self._evaluate_weights(temp_similarity, test_users, metric)
                ...
                if score > best_score:
                    best_score = score
                    best_weights = (genre_w, text_w, numeric_w)
                    
                print(f"Weights: Genre={genre_w}, Text={text_w}, Numeric={numeric_w} -> Score: {score:.3f}")
                
            except Exception as e:
                print(f"Lỗi với weights {genre_w}, {text_w}, {numeric_w}: {e}")
                continue
        
        # Lưu kết quả tốt nhất
        if best_weights:
            self.optimal_weights = {
                'genre': best_weights[0],
                'text': best_weights[1], 
                'numeric': best_weights[2]
            }
            print(f"\nTrọng số tối ưu: Genre={best_weights[0]}, Text={best_weights[1]}, Numeric={best_weights[2]}")
            print(f"Score tốt nhất: {best_score:.3f}")
            
            # Sắp xếp kết quả theo score
            results.sort(key=lambda x: x['score'], reverse=True)
            print("\nTop 5 trọng số tốt nhất:")
            for i, result in enumerate(results[:5]):
                print(f"{i+1}. Genre={result['genre_weight']}, Text={result['text_weight']}, "
                      f"Numeric={result['numeric_weight']} -> Score: {result['score']:.3f}")
        
        return best_weights, best_score
    
    def _build_content_similarity_with_weights(self, genre_weight, text_weight, numeric_weight):
        """Xây dựng content similarity với trọng số tùy chỉnh"""
        try:
            # Tạo features với trọng số mới
            text_features = []
            numeric_features = []
            
            for game in self.games_data:
                # Genre features với trọng số tùy chỉnh
                genre_list = game.get('genre', [])
                genre_weighted = ' '.join([genre for genre in genre_list for _ in range(genre_weight)])
                
                # Text features với trọng số tùy chỉnh
                ...
                # Tăng trọng số text features
                text_weighted = ' '.join([publisher, age_rating, platform_str, mode, multiplayer, language_str] * int(text_weight))
                text_feature = f"{genre_weighted} {text_weighted}"
                text_features.append(text_feature)
                
                # Numeric features với trọng số tùy chỉnh
                price = game.get('price', 0)
                price_normalized = np.log10(max(price, 1)) * numeric_weight
                
                capacity = game.get('capacity', 0) * numeric_weight
                
                min_spec = game.get('min_spec', {})
                cpu_name = min_spec.get('cpu', '')
                cpu_score = self.get_cpu_score(cpu_name) * numeric_weight
                
                gpu_name = min_spec.get('gpu', '')
                gpu_score = self.get_gpu_score(gpu_name) * numeric_weight
                
                ram_str = min_spec.get('ram', '0GB')
                try:
                    ram_gb = int(ram_str.replace('GB', '')) * numeric_weight
                except:
                    ram_gb = 0
                
                downloads = game.get('downloads', 0)
                downloads_normalized = np.log10(max(downloads, 1)) * numeric_weight
                
                numeric_feature = [price_normalized, capacity, cpu_score, gpu_score, ram_gb, downloads_normalized]
                numeric_features.append(numeric_feature)
            
            # Tính TF-IDF và similarity
            ...
            combined_matrix = hstack([text_matrix, numeric_sparse])
            
            similarity_matrix = cosine_similarity(combined_matrix)
            return (similarity_matrix + 1) / 2
            
        except Exception as e:
            print(f"Lỗi khi xây dựng similarity: {e}")
            return None
    
    def _evaluate_weights(self, similarity_matrix, test_users, metric='comprehensive'):
        """Đánh giá hiệu suất của trọng số với metrics cải tiến"""
        ...
        try:
            ...
            for user_id in test_users:
                ...
                # Tính content scores cho các games chưa tương tác
                game_scores = {}
                    ...
                    for interacted_game_id in interacted_games:
                        try:
                            ...
                            if 1 <= game_id_int <= len(self.games_data):
                                sim_score = similarity_matrix[game_id_int - 1][game_id - 1]
                ...
                scores_list = list(game_scores.values())
                
                if metric == 'comprehensive':
                    # Comprehensive scoring với nhiều yếu tố
                    # 1. Score range (max - min)
                    score_range = max(scores_list) - min(scores_list)
                    
                    # 2. Score distribution (std deviation)
                    score_std = np.std(scores_list)
                    
                    # 3. High score count (scores > 0.3)
                    high_scores = sum(1 for s in scores_list if s > 0.3)
                    
                    # 4. Score variance (độ phân tán)
                    score_variance = np.var(scores_list)
                    
                    # 5. Top score quality
                    top_scores = sorted(scores_list, reverse=True)[:5]
                    top_quality = np.mean(top_scores) if top_scores else 0
                    
                    # Combined score với weights
                    comprehensive_score = (
                        score_range * 0.3 +      # Range importance
                        score_std * 0.25 +        # Distribution importance  
                        high_scores * 0.2 +       # High score count
                        score_variance * 0.15 +   # Variance importance
                        top_quality * 0.1         # Top quality
                    )
                    
                    all_scores.append(comprehensive_score)
                    
                elif metric == 'precision':
                    # Precision-based: focus on high-quality recommendations
                    high_quality_scores = [s for s in scores_list if s > 0.4]
                    precision_score = len(high_quality_scores) / len(scores_list) if scores_list else 0
                    all_scores.append(precision_score)
                    
                elif metric == 'recall':
                    # Recall-based: focus on coverage
                    covered_games = sum(1 for s in scores_list if s > 0.1)
                    recall_score = covered_games / len(scores_list) if scores_list else 0
                    ...
                    precision = len(high_quality_scores) / len(scores_list) if scores_list else 0
                    recall = covered_games / len(scores_list) if scores_list else 0
    ...
    def build_content_similarity(self, use_optimal_weights=True):
        """Xây dựng ma trận tương đồng content-based với nhiều thuộc tính"""
        try:
            from sklearn.preprocessing import StandardScaler
            import numpy as np
            
            # Sử dụng trọng số tối ưu nếu có
            if use_optimal_weights and self.optimal_weights:
                print("Sử dụng trọng số tối ưu đã được tìm thấy...")
                genre_weight = self.optimal_weights['genre']
                text_weight = self.optimal_weights['text']
                numeric_weight = self.optimal_weights['numeric']
            else:
            ...
            for game in self.games_data:
                ...
                # Kết hợp với trọng số: Genre (3x), các thuộc tính khác (1.5x)
                # Tăng trọng số cho text features bằng cách lặp lại theo text_weight
                text_weighted = ' '.join([publisher, age_rating, platform_str, mode, multiplayer, language_str] * int(text_weight))
                text_feature = f"{genre_weighted} {text_weighted}"
                text_features.append(text_feature)
                
                # === NUMERIC FEATURES với trọng số động ===
                # Trọng số cho technical specs và downloads
                tech_weight_factor = numeric_weight
                ...
                # Downloads - trọng số động
                downloads = game.get('downloads', 0)
                # Chuẩn hóa downloads về log scale với trọng số động
                downloads_normalized = np.log10(max(downloads, 1)) * numeric_weight
            ...
            for game in self.games_data:
                genre_list = game.get('genre', [])
                # Tạo genre string với trọng số động
                genre_weighted = ' '.join([genre for genre in genre_list for _ in range(genre_weight)])
            ...
            # Kết hợp: Genre (3x weight) + Text (1.5x weight) + Numeric (1x weight) + Downloads (1x weight)
            from scipy.sparse import hstack, csr_matrix
            numeric_sparse = csr_matrix(numeric_matrix)
            # Kết hợp với trọng số: Genre (3x), Text (1.5x), Numeric (1x), Downloads (1x)
            ...
            print(f"Genre features: {genre_matrix.shape[1]} dimensions ({genre_weight}x weight)")
            print(f"Text features: {text_matrix.shape[1]} dimensions ({text_weight}x weight)")
            print(f"Numeric features: {numeric_matrix.shape[1]} dimensions ({numeric_weight}x weight)") 
            print(f"Total features: {combined_matrix.shape[1]} dimensions")
            print("Content similarity normalized to 0-1 range")
            print(f"Dynamic weights: Genre {genre_weight}x, Text {text_weight}x, Numeric {numeric_weight}x!")
            if use_optimal_weights and self.optimal_weights:
                print("Using optimized weights for better performance!")
            return True
        except Exception as e:
...
def main():
    ...
    parser.add_argument('--optimize', action='store_true', help='Optimize content similarity weights')
    ...
    # Kiểm tra nếu có argument --optimize
    if args.optimize:
        print("Bắt đầu tối ưu hóa trọng số...")
        # Tối ưu hóa trọng số
        best_weights, best_score = recommender.optimize_content_weights(metric='diversity')
        
        if best_weights:
            print(f"\nTối ưu hóa hoàn thành!")
            print(f"Trọng số tốt nhất: Genre={best_weights[0]}, Text={best_weights[1]}, Numeric={best_weights[2]}")
            print(f"Score: {best_score:.3f}")
            
            # Xây dựng content similarity với trọng số tối ưu
            recommender.build_content_similarity(use_optimal_weights=True)
        else:
            print("Không tìm thấy trọng số tối ưu, sử dụng trọng số mặc định!")
            recommender.build_content_similarity(use_optimal_weights=False)
    else:
        # Sử dụng trọng số mặc định
        recommender.build_content_similarity(use_optimal_weights=False)
