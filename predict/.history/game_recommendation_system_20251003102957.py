...
class GameRecommendationSystem:
    ...
    def create_content_comparison_chart(self, user_id, recommendations, filename=None):
        ...
        # === SUBPLOT 2: Phân loại theo loại tương tác ===
        favorite_games_data = [game for game in interacted_game_details if 'Favorite' in game['interaction_type']]
        purchased_games_data = [game for game in interacted_game_details if 'Purchased' in game['interaction_type']]
        both_games_data = [game for game in interacted_game_details if len(game['interaction_type'].split(' + ')) > 1]
        
        categories = ['Favorite Only', 'Purchased Only', 'Both Favorite & Purchased']
        counts = [len(favorite_games_data), len(purchased_games_data), len(both_games_data)]
        colors = ['#FF6B6B', '#4ECDC4', '#45B7D1']
        
        bars = ax2.bar(categories, counts, color=colors, alpha=0.8)
        ax2.set_ylabel('Số lượng Games')
        ax2.set_title('Phân loại Games theo Loại Tương tác', fontweight='bold')
