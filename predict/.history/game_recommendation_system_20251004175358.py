...
class GameRecommendationSystem:
    ...
    def get_hybrid_recommendations(self, user_id, top_n=10, keyword=""):
        ...
        for game_id in all_games:
            ...
            # Min-max normalization cho SVD scores (0-1)
            # Tìm min và max của tất cả SVD scores
            all_svd_scores = [all_games[gid]['svd_score'] for gid in all_games if all_games[gid]['svd_score'] != 0]
            if all_svd_scores and svd_score != 0:
                svd_min = min(all_svd_scores)
                svd_max = max(all_svd_scores)
                svd_range = svd_max - svd_min
                if svd_range > 0:
                    svd_normalized = (svd_score - svd_min) / svd_range
                else:
                    svd_normalized = 1.0  # Nếu tất cả scores bằng nhau
            else:
                svd_normalized = 0
