...
class GameRecommendationSystem:
    ...
    def build_content_similarity(self):
        ...
        try:
            ...
            for game in self.games_data:
                ...
                # Release year (chuẩn hóa)
                release_year = game.get('release_year', 2020)
    ...
    def get_keyword_score(self, game, keyword, debug=False):
        ...
        # Tìm trong release_year (nếu keyword chứa số năm)
        ...
        if debug:
            ...
            if release_year_matched:
                print(f"  release_year (+2.0): {game.get('release_year', 0)} matches {keyword}")
            else:
                print(f"  release_year (0): {game.get('release_year', 0)} - no match")
        ...
        # 3.0 + 2.0 + 2.5 + 1.5 + 1.5 + 1.5 + 1.0 + 1.0 + (3 specs * 1.5) + 1.0 + 2.0 (release_year) + 1.0 (multiplayer) = 15.0
    ...
    def get_hybrid_recommendations(self, user_id, top_n=10, keyword=""):
        ...
        if user_data:
            ...
            purchased_games = set(user_data.get('purchased_games', []))
    ...
    def create_content_comparison_chart(self, user_id, recommendations, filename=None):
        ...
        bar_width = 0.15  # Width cố định cho mỗi thanh
        group_spacing = 0.1  # Khoảng cách giữa các nhóm
        
        # Màu sắc đẹp mắt và dễ phân biệt
        colors = ['#E74C3C', '#3498DB', '#2ECC71', '#F39C12', '#9B59B6', '#1ABC9C', '#E67E22']
        total_color = '#34495E'  # Màu xanh đậm cho Total Score
        
        bars = []
        
        # Tính vị trí bắt đầu cho từng nhóm thanh
        total_width = (n_user_games + 1) * bar_width  # +1 cho Total Score
        ...
        for j, user_game in enumerate(user_game_details):
            ...
            bar = ax.bar(x + offset, similarities, bar_width, 
                        label=f"{user_game['game_name'][:20]}",
                        color=colors[j % len(colors)], alpha=0.85, edgecolor='white', linewidth=1.5)
            bars.append(bar)
            
            # Add value labels chỉ khi giá trị đủ cao
            for i, sim in enumerate(similarities):
                if sim > 0.08:  # Chỉ hiển thị nếu > 0.08
                    ax.text(i + offset, sim + 0.015, f'{sim:.2f}', 
                           ha='center', va='bottom', fontsize=9, fontweight='bold', color='black')
        
        # Vẽ thanh cho Total Content Score (Average)
        offset_total = start_offset + n_user_games * bar_width
        total_bar = ax.bar(x + offset_total, total_content_scores, bar_width, 
                          label="★ Avg Similarity",
                          color=total_color, alpha=0.95, linewidth=2.5, edgecolor='gold')
        bars.append(total_bar)
        
        # Add value labels cho Total Score với style đặc biệt
        for i, total_score in enumerate(total_content_scores):
            ax.text(i + offset_total, total_score + 0.025, f'{total_score:.3f}', 
                   ha='center', va='bottom', fontsize=11, fontweight='bold', color='white',
                   bbox=dict(boxstyle="round,pad=0.35", facecolor=total_color, alpha=0.9, edgecolor='gold', linewidth=2))
        
        # Customize chart với style đẹp hơn
        ax.set_xlabel('Top 5 Recommended Games', fontsize=14, fontweight='bold', color='#2C3E50')
        ax.set_ylabel('Content Similarity Score', fontsize=14, fontweight='bold', color='#2C3E50')
        ax.set_title(f'Content-Based Similarity Analysis\nUser: {target_user["name"]} (ID: {user_id})', 
                    fontsize=16, fontweight='bold', pad=25, color='#2C3E50')
        
        # Set x-axis labels với font lớn hơn
        recommended_names = [rec['game_name'][:25] for rec in top_5_recommendations]
        ax.set_xticks(x)
        ax.set_xticklabels(recommended_names, rotation=35, ha='right', fontsize=11, fontweight='500')
        ...
        # Add legend với style đẹp hơn, đặt ở vị trí tốt hơn
        legend = ax.legend(loc='upper left', fontsize=10, framealpha=0.95, 
                          edgecolor='black', fancybox=True, shadow=True,
                          title='User\'s Games', title_fontsize=11)
        ...
        # Add thông tin footer
        user_games_list = ', '.join([f"{game['game_name'][:12]}" for game in user_game_details])
        footer_text = f"📊 Comparing recommended games with user's favorites: {user_games_list}"
