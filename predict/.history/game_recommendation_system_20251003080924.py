...
class GameRecommendationSystem:
    ...
    def create_content_comparison_chart(self, user_id, recommendations, filename=None):
        """Tạo đồ thị so sánh điểm content với từng game user đã thích/tải"""
        if not MATPLOTLIB_AVAILABLE:
        ...
        # Lấy thông tin chi tiết của các games đã tương tác
        interacted_game_details = []
        for game_id in set(interacted_games):  # Loại bỏ trùng lặp
            game = next((g for g in self.games_data if g['id'] == game_id), None)
            if game:
                # Tính content similarity với chính nó (sẽ là 1.0)
                self_similarity = 1.0
                
                # Tính content similarity với các games khác user đã tương tác
                similarities_with_other_games = []
                for other_game_id in set(interacted_games):
                    if other_game_id != game_id:
                        if other_game_id <= len(self.games_data):
                            sim_score = self.content_similarity_matrix[game_id - 1][other_game_id - 1]
                            similarities_with_other_games.append(sim_score)
                
                avg_similarity_with_others = np.mean(similarities_with_other_games) if similarities_with_other_games else 0
                ...
                interacted_game_details.append({
                    'game_id': game_id,
                    'game_name': game['name'],
                    'self_similarity': self_similarity,
                    'avg_similarity_with_others': avg_similarity_with_others,
        ...
        if not interacted_game_details:
            print("Không có dữ liệu games đã tương tác")
            return None
