...
class GameRecommendationSystem:
    ...
    def display_recommendations(self, recommendations, title="Game Recommendations"):
        ...
        for i, rec in enumerate(recommendations, 1):
            ...
            if 'can_run' in rec:
                status = "Co the chay" if rec['can_run'] else "Khong the chay"
                print(f"   Hardware: {status} - {rec['performance_level']}")
...
def get_user_input():
    ...
    users = [
        {"id": 1, "name": "Gamer Pro", "age": 25, "favorite": [1,2,3], "purchased": [1,3,4]},
        {"id": 2, "name": "Casual Player", "age": 35, "favorite": [5,6], "purchased": [5,6]},
        {"id": 3, "name": "Mobile Gamer", "age": 22, "favorite": [6], "purchased": [6]}
    ]
    
    for user in users:
        print(f"{user['id']}. {user['name']} ({user['age']} tuoi)")
        print(f"   Yeu thich: {user['favorite']}, Da mua: {user['purchased']}")
    ...
    # Chon co muon nhap thong tin may tinh khong
    hardware_input = input("\nBan co muon nhap thong tin cau hinh may tinh? (y/n): ").lower()
    
    cpu = gpu = ram = None
    if hardware_input == 'y':
        print("\nCac CPU pho bien:")
        print("Intel i3, Intel i5, Intel i7, Intel i9")
        print("AMD Ryzen 3, AMD Ryzen 5, AMD Ryzen 7, AMD Ryzen 9")
        cpu = input("Nhap CPU cua ban (vi du: Intel i5): ").strip()
        
        print("\nCac GPU pho bien:")
        print("GTX 1050, GTX 1060, GTX 1070, RTX 2060, RTX 3060, RTX 3070, RTX 4060, RTX 4070")
        gpu = input("Nhap GPU cua ban (vi du: GTX 1060): ").strip()
        
        print("\nCac tuy chon RAM:")
        print("4GB, 8GB, 16GB, 32GB")
        ram = input("Nhap RAM cua ban (vi du: 8GB): ").strip()
    
    # Chon so luong goi y
    while True:
        try:
            top_n = int(input("\nSo luong game goi y (3-10): "))
            if 3 <= top_n <= 10:
                break
            else:
                print("Vui long chon tu 3 den 10")
        except ValueError:
            print("Vui long nhap so")
    
    return user_id, cpu, gpu, ram, top_n
...
def main():
    ...
    while True:
        # Lay input tu user
        user_id, cpu, gpu, ram, top_n = get_user_input()
        ...
        print(f"\n" + "="*60)
        print(f"GOI Y CHO: {user_data['name']}")
        print("="*60)
        
        # Hien thi thong tin user
        print(f"Tuoi: {user_data['age']}")
        print(f"Game yeu thich: {user_data.get('favorite_games', [])}")
        print(f"Game da mua: {user_data.get('purchased_games', [])}")
        
        if cpu and gpu and ram:
            print(f"Cau hinh may: CPU {cpu}, GPU {gpu}, RAM {ram}")
        ...
        # Hien thi ket qua
        recommender.display_recommendations(recommendations, f"GOI Y GAME CHO {user_data['name']}")
        
        # Hoi co muon tiep tuc khong
        continue_choice = input("\nBan co muon thu voi user khac? (y/n): ").lower()
        if continue_choice != 'y':
            break
    
    print("\nCam on ban da su dung he thong goi y game!")
    return recommender
