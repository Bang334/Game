            ...
            try:
                ...
                if hasattr(recommender, 'svd_model') and recommender.svd_model is not None:
                    retrain_info['model_parameters']['svd'] = {
                        'n_factors': recommender.svd_model.n_factors,
                        'n_epochs': recommender.svd_model.n_epochs,
                        'lr_all': recommender.svd_model.lr_all,
                        'reg_all': recommender.svd_model.reg_all,
                ...
                if hasattr(recommender, 'user_item_matrix') and recommender.user_item_matrix is not None:
                    retrain_info['model_parameters']['user_item_matrix'] = {
                        'shape': list(recommender.user_item_matrix.shape),
                        'sparsity': float(1 - (recommender.user_item_matrix != 0).sum() / recommender.user_item_matrix.size),
                        'mean_rating': float(recommender.user_item_matrix[recommender.user_item_matrix != 0].mean()) if (recommender.user_item_matrix != 0).sum() > 0 else 0
                    }
