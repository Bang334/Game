            ...
            try:
                ...
                if hasattr(recommender, 'user_item_matrix') and recommender.user_item_matrix is not None:
                    ...
                    if non_zero_count > 0:
                        ...
                        retrain_info['model_parameters']['user_item_matrix']['mean_rating'] = float(np.mean(non_zero_values))
                        retrain_info['model_parameters']['user_item_matrix']['min_rating'] = float(np.min(non_zero_values))
                        retrain_info['model_parameters']['user_item_matrix']['max_rating'] = float(np.max(non_zero_values))
                ...
                if hasattr(recommender, 'content_similarity_matrix') and recommender.content_similarity_matrix is not None:
                    retrain_info['model_parameters']['content_similarity'] = {
                        ...
                        'mean_similarity': float(recommender.content_similarity_matrix.mean().item()),
                        'max_similarity': float(recommender.content_similarity_matrix.max().item()),
                        'min_similarity': float(recommender.content_similarity_matrix.min().item())
                    }
