...
def get_smart_recommendations(user_id, top_n=None, keyword=None):
        ...
        for rec in basic_recs:
            ...
            if game_info:
                ...
                if game_info.get('name'):
                    text_fields.append(str(game_info['name']))
                ...
                if game_info.get('description'):
                    text_fields.append(str(game_info['description']))
                
                # Publisher
                if game_info.get('publisher'):
                    text_fields.append(str(game_info['publisher']))
                ...
                # Mode, Multiplayer, Language
                for field in ['mode', 'multiplayer', 'language']:
                    if game_info.get(field):
                        text_fields.append(str(game_info[field]))
                
                # Tính keyword score
                text_content = ' '.join(text_fields).lower()
                if keyword_lower in text_content:
                    keyword_count = text_content.count(keyword_lower)
                    keyword_score = min(keyword_count / len(text_content.split()) * 10, 1.0)
                    rec['keyword_score'] = keyword_score
                else:
                    rec['keyword_score'] = 0.0
            else:
                rec['keyword_score'] = 0.0
    else:
        # Không có keyword, set keyword_score = 0
        for rec in basic_recs:
            rec['keyword_score'] = 0.0
    ...
    # Tính lại hybrid_score với keyword_score
    for rec in basic_recs:
        ...
        # Tính hybrid score với trọng số động
        rec['hybrid_score'] = (
            dynamic_weights['svd'] * svd_score +
            dynamic_weights['content'] * content_score +
            dynamic_weights['demographic'] * demographic_score +
            dynamic_weights['keyword'] * keyword_score
        )
    
    # Sắp xếp lại theo hybrid score
    basic_recs.sort(key=lambda x: x['hybrid_score'], reverse=True)
    
    # Debug: In thông tin learning (chỉ khi có learning data)
    if user_id in learning_data:
        user_data = learning_data[user_id]
        behavior = user_data.get('behavior_patterns', {})
        
        print(f"\n🧠 Smart Learning for User {user_id}:")
        print(f"   📊 Dynamic Weights: {dynamic_weights}")
        print(f"   🎮 Preferred Genres: {behavior.get('preferred_genres', {})}")
        print(f"   🆕 Prefers New Games: {behavior.get('prefers_new_games', False)}")
        print(f"   💰 Price Tolerance: {behavior.get('price_tolerance', {})}")
        print(f"   ⚡ Recent Activity: {behavior.get('recent_activity_score', 0):.3f}")
        print(f"   📈 Engagement: {behavior.get('engagement_level', {})}")
    ...
    try:
        ...
        for game in all_games:
            ...
            if keyword in text_content:
                # Tính keyword score (số lần xuất hiện / độ dài text)
