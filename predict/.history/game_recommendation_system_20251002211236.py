...
class GameRecommendationSystem:
    ...
    def get_demographic_recommendations(self, user_id, top_n=5):
        ...
        try:
            ...
            # Normalize demographic scores về 0-1 để phân bổ rộng hơn
            if game_scores:
                ...
                if max_score > min_score:
                    for game_id in game_scores:
                        game_scores[game_id] = (game_scores[game_id] - min_score) / (max_score - min_score)
                else:
                    # Nếu tất cả scores bằng nhau, set về 0.5
                    for game_id in game_scores:
                        game_scores[game_id] = 0.5
            
            # Sắp xếp theo điểm đã normalize
            sorted_games = sorted(game_scores.items(), key=lambda x: x[1], reverse=True)
