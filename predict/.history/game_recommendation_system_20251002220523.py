...
class GameRecommendationSystem:
    ...
    def create_scores_chart(self, recommendations, user_data, keyword="", top_n=15):
        ...
        # Save chart
        chart_filename = f'game_scores_chart_user_{user_data["id"]}.png'
    ...
    def create_user_similarity_heatmap(self, user_id, filename=None):
        ...
        ax = sns.heatmap(similarity_matrix, 
                        ...
                        fmt='.3f',
                        cmap='RdYlBu_r',
                        xticklabels=[f"User {uid}\n{name[:10]}" for uid, name in zip(user_ids, user_names)],
                        yticklabels=[f"User {uid}\n{name[:10]}" for uid, name in zip(user_ids, user_names)],
        ...
        # Save heatmap
        if not filename:
            filename = f'user_similarity_heatmap_user_{user_id}.png'
    ...
    def export_scores_to_txt(self, recommendations, user_data, keyword="", filename=None):
        ...
        if not filename:
            filename = f"scores_table_user_{user_data['id']}.txt"
