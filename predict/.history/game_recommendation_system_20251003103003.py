...
class GameRecommendationSystem:
    ...
    def create_content_comparison_chart(self, user_id, recommendations, filename=None):
        ...
        # === SUBPLOT 3: Scatter plot: Content Similarity vs Rating ===
        ratings = [game['rating'] for game in interacted_game_details]
        similarities = [game['avg_similarity_with_others'] for game in interacted_game_details]
        interaction_types = [game['interaction_type'] for game in interacted_game_details]
        
        # Tạo màu sắc khác nhau cho từng loại tương tác
        color_map = {'Favorite': '#FF6B6B', 'Purchased': '#4ECDC4', 'Favorite + Purchased': '#45B7D1'}
        colors = [color_map.get(interaction_type, '#96CEB4') for interaction_type in interaction_types]
        
        scatter = ax3.scatter(ratings, similarities, c=colors, alpha=0.7, s=100)
        ax3.set_xlabel('Game Rating')
        ax3.set_ylabel('Avg Content Similarity with Other Games')
        ...
        legend_elements = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=color, 
                                     markersize=10, label=interaction_type) 
                          for interaction_type, color in color_map.items()]
        ax3.legend(handles=legend_elements, loc='upper right')
        
        # === SUBPLOT 4: Content Similarity Heatmap ===
        # Tạo ma trận similarity giữa các games user đã tương tác
        n_games = len(interacted_game_details)
        similarity_matrix = np.zeros((n_games, n_games))
        
        for i, game1 in enumerate(interacted_game_details):
            for j, game2 in enumerate(interacted_game_details):
                if i == j:
                    similarity_matrix[i][j] = 1.0
                else:
                    sim_score = self.content_similarity_matrix[game1['game_id'] - 1][game2['game_id'] - 1]
                    similarity_matrix[i][j] = sim_score
        
        im = ax4.imshow(similarity_matrix, cmap='RdYlBu_r', aspect='auto', vmin=0, vmax=1)
        ax4.set_xticks(range(n_games))
        ax4.set_xticklabels([game['game_name'][:10] for game in interacted_game_details], 
                           rotation=45, ha='right')
        ax4.set_yticks(range(n_games))
        ax4.set_yticklabels([game['game_name'][:10] for game in interacted_game_details])
        ax4.set_title('Content Similarity Matrix\nBetween User\'s Interacted Games', fontweight='bold')
