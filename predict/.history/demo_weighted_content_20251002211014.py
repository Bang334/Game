...
def demo_weighted_content_score():
    ...
    print("=== DEMO: CONTENT SCORE VỚI TRỌNG SỐ ===")
    print()
    ...
    # Cách cũ (không trùng lặp)
    old_way = list(set(favorite_games + purchased_games))
    print("CÁCH CŨ (Không trùng lặp):")
    print(f"  Combined: {old_way}")
    print(f"  Số lần tính similarity: {len(old_way)} lần")
    print("  → Games [1,3] chỉ được tính 1 lần dù vừa thích vừa mua")
    print()
    
    # Cách mới (giữ trùng lặp)
    new_way = favorite_games + purchased_games
    print("CÁCH MỚI (Giữ trùng lặp - WEIGHTED):")
    print(f"  Combined: {new_way}")
    print(f"  Số lần tính similarity: {len(new_way)} lần")
    print("  → Game 1 (Cyberpunk): Tính 2 lần (thích + mua)")
    print("  → Game 2 (Candy Crush): Tính 1 lần (chỉ thích)")  
    print("  → Game 3 (Call of Duty): Tính 2 lần (thích + mua)")
    print("  → Game 4 (Minecraft): Tính 1 lần (chỉ mua)")
    print()
