...
class GameRecommendationSystem:
    ...
    def create_content_comparison_chart(self, user_id, recommendations, filename=None):
        ...
        footer_text = (f"📊 Matrix shows content similarity between recommended games and user's favorites\n"
                      f"❤️ = Favorite  💰 = Purchased  |  Colors: 🟥 Low → 🟨 Medium → 🟩 High Similarity")
        plt.figtext(0.5, 0.01, footer_text, ha='center', fontsize=10,
                   style='italic', color='#7F8C8D',
                   bbox=dict(boxstyle="round,pad=0.5", facecolor="#ECF0F1", alpha=0.85, edgecolor='#BDC3C7'))
