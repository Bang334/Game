...
class GameRecommendationSystem:
    ...
    def get_keyword_score(self, game, keyword, debug=False):
        ...
        if debug:
            ...
            print(f"Final score: {score} / 16.5 = {final_score:.3f}")
            print("=" * 50)
        
        # Chuẩn hóa score về 0-1 (max possible score ≈ 16.5)
        # 3.0 + 2.0 + 2.5 + 1.5 + 1.5 + 1.5 + 1.0 + 1.0 + (4 specs * 1.5) + 1.0 = 16.5
        return min(score / 16.5, 1.0)
...
def show_recommendations_for_user(recommender, user_id, query=""):
    ...
    recommendations = recommender.get_hybrid_recommendations(
        user_id=user_id,
        top_n=available_games,  # Lay tat ca games
        keyword=query
    )
    ...
    # Vẽ đồ thị
    plot_recommendations_chart(recommendations, user_data['name'], query)

def plot_recommendations_chart(recommendations, user_name, keyword=""):
    """Vẽ đồ thị 4 trọng số điểm cho các games"""
    if not recommendations:
        print("Khong co du lieu de ve do thi.")
        return
    
    # Lấy top 15 games để đồ thị không quá đông
    top_games = recommendations[:15]
    
    # Chuẩn bị dữ liệu
    game_names = [rec['game_name'] for rec in top_games]
    svd_scores = [rec['svd_score'] for rec in top_games]
    content_scores = [rec['content_score'] for rec in top_games]
    demographic_scores = [rec['demographic_score'] for rec in top_games]
    keyword_scores = [rec['keyword_score'] for rec in top_games]
    ...
    # Vẽ 4 cột cho mỗi game
    bars1 = ax.bar(x - 1.5*width, svd_scores, width, label='SVD Score', color='#FF6B6B', alpha=0.8)
    bars2 = ax.bar(x - 0.5*width, content_scores, width, label='Content Score', color='#4ECDC4', alpha=0.8)
    bars3 = ax.bar(x + 0.5*width, demographic_scores, width, label='Demographic Score', color='#45B7D1', alpha=0.8)
    bars4 = ax.bar(x + 1.5*width, keyword_scores, width, label='Keyword Score', color='#96CEB4', alpha=0.8)
    ...
    add_value_labels(bars1, svd_scores)
    add_value_labels(bars2, content_scores)
    add_value_labels(bars3, demographic_scores)
    add_value_labels(bars4, keyword_scores)
    
    # Cấu hình đồ thị
    ax.set_xlabel('Games', fontsize=12, fontweight='bold')
    ax.set_ylabel('Scores (0-1)', fontsize=12, fontweight='bold')
    
    title = f'Game Recommendation Scores - {user_name}'
    if keyword:
    ...
    ax.set_ylim(0, 1.1)  # Giới hạn trục y từ 0 đến 1.1
    
    # Hiển thị trọng số
    if keyword and keyword.strip():
        weights = WEIGHTS_WITH_KEYWORD
        weight_text = f"Weights: SVD({weights['svd']:.0%}) + Content({weights['content']:.0%}) + Demographic({weights['demographic']:.0%}) + Keyword({weights['keyword']:.0%})"
    else:
        weights = WEIGHTS_NO_KEYWORD
        weight_text = f"Weights: SVD({weights['svd']:.0%}) + Content({weights['content']:.0%}) + Demographic({weights['demographic']:.0%})"
    
    ax.text(0.02, 0.98, weight_text, transform=ax.transAxes, fontsize=10,
            verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
    
    plt.tight_layout()
    
    # Lưu đồ thị
    filename = f"recommendations_{user_name.replace(' ', '_')}"
    if keyword:
        filename += f"_{keyword.replace(' ', '_')}"
    filename += ".png"
    
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"\nDo thi da duoc luu vao file: {filename}")
