...
class GameRecommendationSystem:
    ...
    def create_content_comparison_chart(self, user_id, recommendations, filename=None):
        """Tạo biểu đồ đường so sánh top 5 games được gợi ý với từng game user đã thích/tải"""
        if not MATPLOTLIB_AVAILABLE:
        ...
        # Trục X cho line chart
        x = np.arange(n_recommended)
        
        # Màu sắc đẹp mắt cho các đường
        colors = ['#E74C3C', '#3498DB', '#2ECC71', '#F39C12', '#9B59B6', '#1ABC9C', '#E67E22']
        markers = ['o', 's', '^', 'D', 'v', 'p', '*']
        
        # Vẽ đường cho từng user game
        for j, user_game in enumerate(user_game_details):
            similarities = [similarity_data[i][j] for i in range(n_recommended)]
            
            # Vẽ line với marker
            ax.plot(x, similarities, 
                   color=colors[j % len(colors)], 
                   marker=markers[j % len(markers)],
                   markersize=8,
                   linewidth=2.5,
                   label=f"{user_game['game_name'][:22]} {user_game['interaction_type']}",
                   alpha=0.8)
            
            # Add value labels tại các điểm nếu giá trị đủ cao
            for i, sim in enumerate(similarities):
                if sim > 0.1:  # Chỉ hiển thị nếu > 0.1
        ...
        # Vẽ đường Average Similarity (đặc biệt)
        ax.plot(x, total_content_scores,
               color='#2C3E50',
               marker='*',
               markersize=14,
               linewidth=4,
               ...
               alpha=0.95,
               linestyle='--',
               zorder=10)  # Vẽ trên cùng
        
        # Add value labels cho Average line
        for i, total_score in enumerate(total_content_scores):
            ax.annotate(f'{total_score:.3f}', 
                       xy=(i, total_score), 
                       xytext=(0, 12),
                       textcoords='offset points',
                       ha='center',
        ...
        ax.set_title(f'Content-Based Similarity Analysis (Line Chart)\nUser: {target_user["name"]} (ID: {user_id})', 
                    fontsize=16, fontweight='bold', pad=25, color='#2C3E50')
        
        # Set x-axis labels
        recommended_names = [f"{i+1}. {rec['game_name'][:25]}" for i, rec in enumerate(top_5_recommendations)]
        ax.set_xticks(x)
        ax.set_xticklabels(recommended_names, rotation=25, ha='right', fontsize=11)
        ...
        user_games_list = ', '.join([f"{game['game_name'][:15]}" for game in user_game_details])
        footer_text = f"📊 Comparing recommended games similarity with user's favorites: {user_games_list}\n❤️ = Favorite  💰 = Purchased"
        plt.figtext(0.5, 0.015, footer_text, ha='center', fontsize=10, 
                   style='italic', color='#7F8C8D',
                   bbox=dict(boxstyle="round,pad=0.6", facecolor="#ECF0F1", alpha=0.85, edgecolor='#BDC3C7', linewidth=1.5))
        ...
        print(f"Content comparison chart (line) saved to: {filename}")
