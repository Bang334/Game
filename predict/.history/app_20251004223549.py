        ...
        if recommender:
            try:
                ...
                if hasattr(recommender, 'user_item_matrix') and recommender.user_item_matrix is not None:
                    ...
                    non_zero_count = (matrix != 0).sum()
                    total_count = matrix.size
                    
                    retrain_info['model_parameters']['user_item_matrix'] = {
                        'shape': list(matrix.shape),
                        'sparsity': float(1 - non_zero_count / total_count) if total_count > 0 else 0,
                        'non_zero_entries': int(non_zero_count),
                        'total_entries': int(total_count)
                    }
                    
                    # Mean rating chỉ tính nếu có dữ liệu - sử dụng numpy để tránh lỗi pandas
                    if non_zero_count > 0:
                        import numpy as np
                        non_zero_values = matrix[matrix != 0]
                        # Chuyển đổi sang numpy array để tránh lỗi pandas
                        non_zero_array = np.array(non_zero_values)
                        retrain_info['model_parameters']['user_item_matrix']['mean_rating'] = float(np.mean(non_zero_array))
                        retrain_info['model_parameters']['user_item_matrix']['min_rating'] = float(np.min(non_zero_array))
                        retrain_info['model_parameters']['user_item_matrix']['max_rating'] = float(np.max(non_zero_array))
                ...
                if hasattr(recommender, 'content_similarity_matrix') and recommender.content_similarity_matrix is not None:
                    ...
                    retrain_info['model_parameters']['content_similarity'] = {
                        ...
                        'mean_similarity': float(np.mean(recommender.content_similarity_matrix)),
                        'max_similarity': float(np.max(recommender.content_similarity_matrix)),
                        'min_similarity': float(np.min(recommender.content_similarity_matrix))
                    }
