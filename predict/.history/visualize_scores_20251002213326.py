...
def get_scores_for_visualization(user_id=2):
    """Lấy điểm số từ hệ thống recommendation để vẽ biểu đồ"""
    print("Khởi tạo hệ thống để lấy điểm số...")
    ...
    print(f"Lấy recommendations cho user {user_id}...")
    
    # Lấy recommendations với tất cả điểm số
    user_data = next((u for u in recommender.users_data if u['id'] == user_id), None)
    if not user_data:
        print(f"Không tìm thấy user {user_id}")
        return None
    
    # Lấy số games có thể gợi ý
    ...
    # Lấy hybrid recommendations với tất cả games
    recommendations = recommender.get_hybrid_recommendations(
        user_id=user_id,
        top_n=available_games,  # Lấy tất cả
        keyword=""  # Không có keyword
    )
...
def create_stacked_bar_chart(recommendations, user_data, top_n=15):
    ...
    # Lấy top N games để vẽ (tránh quá đông)
