...
class GameRecommendationSystem:
    ...
    def create_content_comparison_chart(self, user_id, recommendations, filename=None):
        ...
        width = 0.8 / n_user_games  # Điều chỉnh width để fit tất cả bars
        
        # Màu sắc cho từng user game
        ...
        ax.set_title(f'Content Similarity: Top 5 Games vs User\'s Games\n{target_user["name"]} (ID: {user_id})', 
                    fontsize=14, fontweight='bold', pad=20)
        
        # Set x-axis labels
        recommended_names = [rec['game_name'][:20] for rec in top_5_recommendations]
        ax.set_xticks(x + width * (n_user_games - 1) / 2)
        ...
        explanation = f"User Games: {', '.join([game['game_name'][:10] for game in user_game_details])}"
