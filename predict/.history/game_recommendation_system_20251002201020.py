...
class GameRecommendationSystem:
    ...
    def get_keyword_score(self, game, keyword):
        ...
        for field, weight in searchable_fields.items():
            ...
            # Tìm kiếm với tất cả keywords đã mở rộng
        ...
        for spec_field in spec_fields:
            ...
            if keyword in min_val or keyword in rec_val:
                score += 1.5  # Specs có trọng số trung bình
        
        # Tìm trong price (nếu keyword là số)
        try:
            keyword_num = float(keyword.replace('gb', '').replace('mb', '').replace('k', '000').replace('m', '000000'))
            price = game.get('price', 0)
    ...
    def get_hybrid_recommendations(self, user_id, top_n=10, keyword=""):
        ...
        for game_id in all_games:
            # Điều chỉnh trọng số khi có keyword
            if keyword and keyword.strip():
                svd_weight = 0.4        # 40% SVD
                content_weight = 0.25   # 25% Content-based
                demographic_weight = 0.15 # 15% Demographic
                keyword_weight = 0.2    # 20% Keyword
            else:
                svd_weight = 0.5        # 50% SVD
                content_weight = 0.3    # 30% Content-based
                demographic_weight = 0.2 # 20% Demographic
...
def show_all_recommendations_with_query(recommender, query=""):
    ...
    for user_id in [1, 2, 3]:
        ...
        recommender.display_recommendations(recommendations, title)
...
def show_recommendations_for_user(recommender, user_id, query=""):
    ...
    recommendations = recommender.get_hybrid_recommendations(
        user_id=user_id,
        top_n=min(10, available_games),  # Gioi han 10 games
        keyword=query
    )
