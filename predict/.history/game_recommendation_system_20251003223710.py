...
class GameRecommendationSystem:
    ...
    def build_content_similarity(self):
        ...
        try:
            ...
            # Sử dụng threshold-based scaling để tạo phân bố tự nhiên
            # Chỉ giữ lại similarity cao, set thấp về 0
            threshold = 0.1  # Chỉ giữ similarity > 0.1
            
            # Tạo similarity matrix mới
            filtered_similarity = np.where(similarity_matrix > threshold, similarity_matrix, 0.0)
            
            # Đảm bảo diagonal = 1 (game giống chính nó)
            np.fill_diagonal(filtered_similarity, 1.0)
            
            # Chuẩn hóa về 0-1 với min-max scaling
            min_sim = filtered_similarity.min()
            max_sim = filtered_similarity.max()
            
            if max_sim > min_sim:
                self.content_similarity_matrix = (filtered_similarity - min_sim) / (max_sim - min_sim)
            else:
                self.content_similarity_matrix = filtered_similarity
            
            print(f"Threshold similarity range: {self.content_similarity_matrix.min():.3f} - {self.content_similarity_matrix.max():.3f}")
            print(f"Threshold similarity mean: {self.content_similarity_matrix.mean():.3f}")
            ...
            print("Content similarity normalized to 0-1 range with threshold filtering!")
            print("All features now have equal weights (1x)!")
            print("Threshold filtering creates natural distribution with many 0.0 scores!")
            return True
        except Exception as e:
