...
class GameRecommendationSystem:
    ...
    def create_content_comparison_chart(self, user_id, recommendations, filename=None):
        ...
        # === SUBPLOT 1: Content Similarity với chính nó ===
        game_names = [game['game_name'][:20] for game in interacted_game_details]
        self_similarities = [game['self_similarity'] for game in interacted_game_details]
        other_similarities = [game['avg_similarity_with_others'] for game in interacted_game_details]
        
        x = np.arange(len(game_names))
        width = 0.35
        
        bars1 = ax1.bar(x - width/2, self_similarities, width, label='Self Similarity (1.0)', 
                       color='#FF6B6B', alpha=0.8)
        bars2 = ax1.bar(x + width/2, other_similarities, width, label='Avg Similarity with Other Games', 
                       color='#4ECDC4', alpha=0.8)
        
        ax1.set_xlabel('Games User Đã Tương Tác')
        ax1.set_ylabel('Content Similarity Score')
        ax1.set_title('Content Similarity: Self vs Other Games', fontweight='bold')
        ax1.set_xticks(x)
        ...
        # Add value labels
        for i, (self_sim, other_sim) in enumerate(zip(self_similarities, other_similarities)):
            ax1.text(i - width/2, self_sim + 0.02, f'{self_sim:.2f}', ha='center', va='bottom', fontsize=8)
            ax1.text(i + width/2, other_sim + 0.02, f'{other_sim:.2f}', ha='center', va='bottom', fontsize=8)
