    ...
    def build_content_similarity(self, use_optimal_weights=True):
        ...
        try:
            ...
            for game in self.games_data:
                ...
                # Tăng trọng số cho text features bằng cách lặp lại 1.5 lần
                text_weighted = f"{publisher} {publisher} {age_rating} {platform_str} {mode} {multiplayer} {language_str}"
                text_feature = f"{genre_weighted} {text_weighted}"
                text_features.append(text_feature)
                
                # === NUMERIC FEATURES với trọng số chuẩn ===
                # Trọng số cho technical specs và downloads
                tech_weight_factor = 1.0  # Trọng số chuẩn cho technical specs
                ...
                # Downloads - trọng số chuẩn
                downloads = game.get('downloads', 0)
                # Chuẩn hóa downloads về log scale với trọng số chuẩn
                downloads_normalized = np.log10(max(downloads, 1)) * 1.0  # Trọng số chuẩn
            ...
            for game in self.games_data:
                genre_list = game.get('genre', [])
                # Tạo genre string với trọng số cao (lặp lại 3 lần)
                genre_weighted = ' '.join([genre for genre in genre_list for _ in range(3)])
            ...
            print(f"Genre features: {genre_matrix.shape[1]} dimensions (3x weight)")
            print(f"Text features: {text_matrix.shape[1]} dimensions (1.5x weight)")
            print(f"Numeric features: {numeric_matrix.shape[1]} dimensions (1x weight)") 
            print(f"Total features: {combined_matrix.shape[1]} dimensions")
            print("Content similarity normalized to 0-1 range")
            print("Genre similarity now has 3x weight, Text 1.5x, Numeric 1x!")
            print("All features now have balanced weights!")
            return True
        except Exception as e:
