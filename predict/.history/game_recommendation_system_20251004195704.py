...
class GameRecommendationSystem:
    ...
    def expand_query(self, query):
        """Thay thế query bằng cách ánh xạ từ tiếng Việt sang tiếng Anh"""
        ...
        # Thêm query gốc
        replaced_keywords.add(query)
        
        # Tìm kiếm trong library để thay thế
        for english_key, vietnamese_synonyms in self.keyword_library.items():
            ...
            for word in query_words:
                ...
                if word in vietnamese_synonyms:
                    # Thay thế bằng english_key thay vì mở rộng
                    replaced_keywords.add(english_key.lower())
        
        # Kết hợp tất cả keywords (có thể có nhiều english_key nếu từ xuất hiện trong nhiều category)
    ...
    def get_keyword_score(self, game, keyword, debug=False):
        ...
        # Tìm trong release_year (nếu keyword là số năm)
        release_year_matched = False
        try:
            original_keyword = keyword.lower().strip()
            keyword_year = int(original_keyword)
            release_year = game.get('release_year', 0)
            
            # Nếu keyword là năm và khớp với release_year
            if release_year > 0 and keyword_year == release_year:
                score += 2.0  # Trọng số cao cho năm chính xác
                release_year_matched = True
            # Nếu keyword là năm gần với release_year (trong khoảng 2 năm)
            elif release_year > 0 and abs(release_year - keyword_year) <= 2:
        ...
        if 'multiplayer' in original_keyword.lower():
            ...
            if game_multiplayer:
                score += 2.0  # Trọng số cao cho multiplayer
        ...
        if debug:
            ...
            if multiplayer_matched:
                print(f"  multiplayer (+2.0): {game.get('multiplayer', False)} matches {keyword}")
            else:
                print(f"  multiplayer (0): {game.get('multiplayer', False)} - no match")
            
            print(f"\nTotal raw score: {score}")
            final_score = min(score / 20.0, 1.0)
            print(f"Final score: {score} / 20.0 = {final_score:.3f}")
            print("=" * 50)
        
        # Chuẩn hóa score về 0-1 (max possible score ≈ 20.0)
        # 3.0 + 2.0 + 2.5 + 1.5 + 1.5 + 1.5 + 1.0 + 1.0 + (3 specs * 1.5) + 1.0 + 2.0 (release_year) + 2.0 (multiplayer) = 20.0
