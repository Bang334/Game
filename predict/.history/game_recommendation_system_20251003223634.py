...
class GameRecommendationSystem:
    ...
    def build_content_similarity(self):
        ...
        try:
            ...
            # Sử dụng sigmoid scaling để có phân bố tự nhiên hơn
            # Sigmoid sẽ tạo ra phân bố từ 0-1 với nhiều giá trị thấp
            import math
            
            # Áp dụng sigmoid function: 1 / (1 + exp(-x))
            # Scale raw similarity trước khi áp dụng sigmoid
            scaled_similarity = similarity_matrix * 2  # Scale để có range rộng hơn
            sigmoid_similarity = 1 / (1 + np.exp(-scaled_similarity))
            
            # Đảm bảo diagonal = 1 (game giống chính nó)
            np.fill_diagonal(sigmoid_similarity, 1.0)
            
            self.content_similarity_matrix = sigmoid_similarity
            
            print(f"Sigmoid similarity range: {self.content_similarity_matrix.min():.3f} - {self.content_similarity_matrix.max():.3f}")
            print(f"Sigmoid similarity mean: {self.content_similarity_matrix.mean():.3f}")
