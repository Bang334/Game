    ...
    def build_content_similarity(self):
        ...
        try:
            ...
            for game in self.games_data:
                ...
                # Trọng số chuẩn cho tất cả features
                tech_weight_factor = 1.0
                ...
                downloads_normalized = np.log10(max(downloads, 1)) * 1.0
            ...
            # Tạo riêng genre features với trọng số chuẩn
            genre_features = []
            for game in self.games_data:
                genre_list = game.get('genre', [])
                # Tạo genre string với trọng số chuẩn (1x)
                genre_weighted = ' '.join(genre_list)
            ...
            # Kết hợp: Genre + Text + Numeric (tất cả trọng số 1x)
            from scipy.sparse import hstack, csr_matrix
            numeric_sparse = csr_matrix(numeric_matrix)
            # Kết hợp với trọng số chuẩn: Genre (1x), Text (1x), Numeric (1x)
            combined_matrix = hstack([genre_matrix, text_matrix, numeric_sparse])
            
            # Tính cosine similarity
            similarity_matrix = cosine_similarity(combined_matrix)
            
            # Debug: In ra một số giá trị similarity để kiểm tra
            print(f"Raw similarity matrix shape: {similarity_matrix.shape}")
            print(f"Raw similarity range: {similarity_matrix.min():.3f} - {similarity_matrix.max():.3f}")
            print(f"Raw similarity mean: {similarity_matrix.mean():.3f}")
            
            # Sử dụng raw similarity values không scale
            # Chỉ set negative values về 0
            raw_similarity = np.where(similarity_matrix < 0, 0.0, similarity_matrix)
            
            # Đảm bảo diagonal = 1 (game giống chính nó)
            np.fill_diagonal(raw_similarity, 1.0)
            
            # Sử dụng raw values trực tiếp
            self.content_similarity_matrix = raw_similarity
            
            print(f"Raw similarity range: {self.content_similarity_matrix.min():.3f} - {self.content_similarity_matrix.max():.3f}")
            print(f"Raw similarity mean: {self.content_similarity_matrix.mean():.3f}")
            print(f"Games with 0 similarity: {np.sum(self.content_similarity_matrix == 0)}")
            print(f"Games with similarity < 0.1: {np.sum(self.content_similarity_matrix < 0.1)}")
            print(f"Games with similarity < 0.3: {np.sum(self.content_similarity_matrix < 0.3)}")
            
            # Debug: Kiểm tra negative values trong raw similarity
            negative_count = np.sum(similarity_matrix < 0)
            print(f"Negative values in raw similarity: {negative_count}")
            if negative_count > 0:
                print(f"Most negative value: {similarity_matrix.min():.3f}")
                print(f"Negative values range: {similarity_matrix[similarity_matrix < 0].min():.3f} to {similarity_matrix[similarity_matrix < 0].max():.3f}")
            
            print("Xay dung content similarity matrix voi nhieu thuoc tinh thanh cong!")
            print(f"Genre features: {genre_matrix.shape[1]} dimensions (1x weight)")
            ...
            print("Content similarity using raw values without scaling!")
            print("All features now have equal weights (1x)!")
            print("Raw similarity values show natural distribution!")
            return True
        except Exception as e:
