...
class GameRecommendationSystem:
    ...
    def build_content_similarity(self):
        ...
        try:
            ...
            # Sử dụng raw similarity values trực tiếp (bao gồm cả âm)
            # Không thay đổi gì, giữ nguyên raw values
            self.content_similarity_matrix = similarity_matrix
            
            print(f"Final similarity range: {self.content_similarity_matrix.min():.3f} - {self.content_similarity_matrix.max():.3f}")
            print(f"Final similarity mean: {self.content_similarity_matrix.mean():.3f}")
            ...
            # Debug: Kiểm tra negative values
            negative_count = np.sum(self.content_similarity_matrix < 0)
            print(f"Negative values count: {negative_count}")
            if negative_count > 0:
                print(f"Most negative value: {self.content_similarity_matrix.min():.3f}")
                print(f"Negative values range: {self.content_similarity_matrix[self.content_similarity_matrix < 0].min():.3f} to {self.content_similarity_matrix[self.content_similarity_matrix < 0].max():.3f}")
            ...
            print("Content similarity using pure raw values (including negative)!")
            print("All features now have equal weights (1x)!")
            print("Pure raw similarity values preserve original cosine similarity!")
            return True
        except Exception as e:
