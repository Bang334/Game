...
class GameRecommendationSystem:
    ...
    def _evaluate_weights(self, similarity_matrix, test_users, metric='diversity'):
        """Đánh giá hiệu suất của trọng số"""
        ...
        try:
            ...
            for user_id in test_users:
                ...
                if metric == 'diversity':
                    # Đánh giá dựa trên độ đa dạng của scores
                    if game_scores:
                        scores_list = list(game_scores.values())
                        diversity = np.std(scores_list)  # Độ lệch chuẩn cao = đa dạng hơn
                        scores.append(diversity)
                elif metric == 'coverage':
                    # Đánh giá dựa trên số lượng games có score > 0
                    coverage = sum(1 for score in game_scores.values() if score > 0.1)
                    scores.append(coverage)
                elif metric == 'quality':
                    # Đánh giá dựa trên chất lượng scores (trung bình)
                    if game_scores:
                        avg_score = np.mean(list(game_scores.values()))
