...
class GameRecommendationSystem:
    ...
    def build_content_similarity(self):
        ...
        try:
            ...
            for game in self.games_data:
                ...
                price_normalized = np.log10(max(price, 1))  # log để giảm scale
    ...
    def create_content_comparison_chart(self, user_id, recommendations, filename=None):
        """Tạo đồ thị so sánh điểm content của từng game được gợi ý với games user đã thích/tải"""
        if not MATPLOTLIB_AVAILABLE:
        ...
        # Lấy top recommendations để phân tích
        top_recommendations = recommendations[:10]  # Top 10 games được gợi ý
        
        if not top_recommendations:
            print("Không có recommendations để phân tích")
            return None
        
        # Tính content similarity của từng game được gợi ý với games user đã tương tác
        recommendation_analysis = []
        for rec in top_recommendations:
            game_id = rec['game_id']
            game_name = rec['game_name']
            
            # Tính similarity với từng game user đã tương tác
            similarities_with_user_games = []
            for interacted_game_id in set(interacted_games):
                if interacted_game_id <= len(self.games_data):
                    sim_score = self.content_similarity_matrix[game_id - 1][interacted_game_id - 1]
                    similarities_with_user_games.append(sim_score)
            
            # Tính trung bình similarity (đây chính là content score)
            avg_similarity = np.mean(similarities_with_user_games) if similarities_with_user_games else 0
            
            # Lấy thông tin chi tiết của game được gợi ý
            game = next((g for g in self.games_data if g['id'] == game_id), None)
            
            recommendation_analysis.append({
                'game_id': game_id,
                'game_name': game_name,
                'avg_similarity_with_user_games': avg_similarity,
                'content_score': rec.get('content_score', 0),  # Content score từ hệ thống
                'hybrid_score': rec.get('hybrid_score', 0),
                'rating': game.get('rating', 0) if game else 0,
                'genre': game.get('genre', []) if game else [],
                'price': game.get('price', 0) if game else 0
            })
        
        if not recommendation_analysis:
            print("Không có dữ liệu recommendations để phân tích")
            return None
        
        # Tạo figure với subplots
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(18, 14))
        
        # === SUBPLOT 1: Content Similarity của Games được gợi ý với User Games ===
        game_names = [rec['game_name'][:20] for rec in recommendation_analysis]
        calculated_similarities = [rec['avg_similarity_with_user_games'] for rec in recommendation_analysis]
        system_content_scores = [rec['content_score'] for rec in recommendation_analysis]
        
        x = np.arange(len(game_names))
        width = 0.35
        
        bars1 = ax1.bar(x - width/2, calculated_similarities, width, label='Calculated Similarity (Manual)', 
                       color='#FF6B6B', alpha=0.8)
        bars2 = ax1.bar(x + width/2, system_content_scores, width, label='System Content Score', 
                       color='#4ECDC4', alpha=0.8)
        
        ax1.set_xlabel('Games Được Gợi Ý')
        ax1.set_ylabel('Content Similarity Score')
        ax1.set_title('Content Similarity: Calculated vs System Score', fontweight='bold')
        ax1.set_xticks(x)
        ...
        # Add value labels
        for i, (calc_sim, sys_sim) in enumerate(zip(calculated_similarities, system_content_scores)):
            ax1.text(i - width/2, calc_sim + 0.02, f'{calc_sim:.3f}', ha='center', va='bottom', fontsize=8)
            ax1.text(i + width/2, sys_sim + 0.02, f'{sys_sim:.3f}', ha='center', va='bottom', fontsize=8)
        
        # === SUBPLOT 2: Top Games với Content Similarity cao nhất ===
        # Sắp xếp theo similarity để hiển thị top games
        sorted_analysis = sorted(recommendation_analysis, key=lambda x: x['avg_similarity_with_user_games'], reverse=True)
        top_games = sorted_analysis[:8]  # Top 8 games
        
        top_game_names = [game['game_name'][:15] for game in top_games]
        top_similarities = [game['avg_similarity_with_user_games'] for game in top_games]
        
        bars = ax2.barh(top_game_names, top_similarities, color='#4ECDC4', alpha=0.8)
        ax2.set_xlabel('Content Similarity Score')
        ax2.set_title('Top Games với Content Similarity Cao Nhất', fontweight='bold')
        ax2.set_xlim(0, 1.1)
        
        # Add value labels
        for i, (bar, sim) in enumerate(zip(bars, top_similarities)):
            ax2.text(sim + 0.02, i, f'{sim:.3f}', va='center', fontsize=9)
        
        # === SUBPLOT 3: Scatter plot: Content Similarity vs Game Rating ===
        ratings = [rec['rating'] for rec in recommendation_analysis]
        similarities = [rec['avg_similarity_with_user_games'] for rec in recommendation_analysis]
        hybrid_scores = [rec['hybrid_score'] for rec in recommendation_analysis]
        
        # Tạo màu sắc dựa trên hybrid score
        colors = ['#FF6B6B' if score > 0.5 else '#4ECDC4' if score > 0.3 else '#96CEB4' 
                 for score in hybrid_scores]
        
        scatter = ax3.scatter(ratings, similarities, c=colors, alpha=0.7, s=100)
        ax3.set_xlabel('Game Rating')
        ax3.set_ylabel('Content Similarity with User Games')
        ax3.set_title('Content Similarity vs Game Rating', fontweight='bold')
        
        # Add trend line
        if len(ratings) > 1:
            z = np.polyfit(ratings, similarities, 1)
            p = np.poly1d(z)
            ax3.plot(sorted(ratings), p(sorted(ratings)), "r--", alpha=0.8, linewidth=2)
            
            # Calculate correlation
            correlation = np.corrcoef(ratings, similarities)[0,1]
            ax3.text(0.05, 0.95, f'Correlation: {correlation:.3f}', 
                    transform=ax3.transAxes, fontsize=10, 
                    bbox=dict(boxstyle="round,pad=0.3", facecolor="yellow", alpha=0.7))
        
        # Add legend for hybrid score levels
        legend_elements = [
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#FF6B6B', 
                      markersize=10, label='High Hybrid Score (>0.5)'),
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#4ECDC4', 
                      markersize=10, label='Medium Hybrid Score (0.3-0.5)'),
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#96CEB4', 
                      markersize=10, label='Low Hybrid Score (<0.3)')
        ]
        ax3.legend(handles=legend_elements, loc='upper right')
        
        # === SUBPLOT 4: Content Similarity Heatmap ===
        # Tạo ma trận similarity giữa games được gợi ý và games user đã tương tác
        n_recommendations = len(recommendation_analysis)
        n_user_games = len(set(interacted_games))
        similarity_matrix = np.zeros((n_recommendations, n_user_games))
        
        # Lấy danh sách user games
        user_game_ids = list(set(interacted_games))
        user_game_names = []
        for game_id in user_game_ids:
            game = next((g for g in self.games_data if g['id'] == game_id), None)
            if game:
                user_game_names.append(game['name'][:10])
        
        for i, rec in enumerate(recommendation_analysis):
            for j, user_game_id in enumerate(user_game_ids):
                sim_score = self.content_similarity_matrix[rec['game_id'] - 1][user_game_id - 1]
                similarity_matrix[i][j] = sim_score
        
        im = ax4.imshow(similarity_matrix, cmap='RdYlBu_r', aspect='auto', vmin=0, vmax=1)
        ax4.set_xticks(range(n_user_games))
        ax4.set_xticklabels(user_game_names, rotation=45, ha='right')
        ax4.set_yticks(range(n_recommendations))
        ax4.set_yticklabels([rec['game_name'][:10] for rec in recommendation_analysis])
        ax4.set_title('Content Similarity Matrix\nRecommended Games vs User\'s Games', fontweight='bold')
        
        # Add text annotations
        for i in range(n_recommendations):
            for j in range(n_user_games):
        ...
        # Overall title
        fig.suptitle(f'Content Similarity Analysis - {target_user["name"]} (ID: {user_id})\n'
                    f'Age: {target_user["age"]}, Gender: {target_user["gender"]} - '
                    f'User Games: {len(set(interacted_games))}, Recommendations: {len(recommendation_analysis)}', 
