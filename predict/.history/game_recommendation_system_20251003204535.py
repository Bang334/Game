...
class GameRecommendationSystem:
    ...
    def preprocess_data(self):
        ...
        # Giả sử: favorite = 5, purchased = 3, view = 1 (mỗi lần xem), có thể cộng dồn
        user_game_ratings = []
        ...
        for user in self.users_data:
            ...
            for game in self.games_data:
                ...
                # Thêm điểm từ view history (1 điểm mỗi lần xem)
                view_count = view_history.count(game_id)
                rating += view_count * 1.0  # 1 điểm mỗi lần xem
                
                # Nếu vừa favorite vừa purchased vừa xem nhiều lần = 5.0 + 3.0 + (số lần xem * 1.0)
    ...
    def get_content_recommendations(self, user_id, top_n=5):
        ...
        try:
            ...
            interacted_games = favorite_games + purchased_games + view_history
...
def show_recommendations_for_user(recommender, user_id, query=""):
    ...
    print(f"Game yeu thich: {user_data.get('favorite_games', [])}")
    print(f"Game da mua: {user_data.get('purchased_games', [])}")
    print(f"Lich su xem: {user_data.get('view_history', [])}")
