...
class GameRecommendationSystem:
    ...
    def build_content_similarity(self):
        ...
        try:
            ...
            # Boost similarity để tăng phân biệt, nhưng giữ negative values
            # Tăng similarity lên 10x để có range rộng hơn và giảm số games = 0
            boosted_similarity = similarity_matrix * 10.0
            
            # Thêm baseline similarity để đảm bảo không có games = 0
            baseline_similarity = 0.1
            boosted_similarity = boosted_similarity + baseline_similarity
            
            # Đảm bảo diagonal = 1 (game giống chính nó)
            np.fill_diagonal(boosted_similarity, 1.0)
            
            # Sử dụng boosted values (bao gồm cả âm)
            self.content_similarity_matrix = boosted_similarity
            
            print(f"Boosted similarity range: {self.content_similarity_matrix.min():.3f} - {self.content_similarity_matrix.max():.3f}")
            print(f"Boosted similarity mean: {self.content_similarity_matrix.mean():.3f}")
            print(f"Games with negative similarity: {np.sum(self.content_similarity_matrix < 0)}")
            ...
            print("Content similarity using 5x boosted values (including negative)!")
            print("All features now have equal weights (1x)!")
            print("5x boost provides much better distribution!")
            return True
        except Exception as e:
