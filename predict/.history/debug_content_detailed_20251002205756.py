...
def get_cpu_score(cpu_name, cpu_benchmarks):
    ...
    for cpu in cpu_benchmarks['cpus']:
        if cpu['name'].lower() in cpu_name or cpu_name in cpu['name'].lower():
...
def get_gpu_score(gpu_name, gpu_benchmarks):
    ...
    for gpu in gpu_benchmarks['gpus']:
        if gpu['name'].lower() in gpu_name or gpu_name in gpu['name'].lower():
