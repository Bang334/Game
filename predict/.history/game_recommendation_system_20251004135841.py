...
class GameRecommendationSystem:
    ...
    def build_content_similarity(self):
        ...
        try:
            ...
            # Đảm bảo tất cả similarity scores đều dương bằng cách cộng với giá trị nhỏ nhất
            min_similarity = similarity_matrix.min()
            if min_similarity < 0:
                # Cộng với giá trị tuyệt đối của min để đảm bảo tất cả >= 0
                similarity_matrix = similarity_matrix + abs(min_similarity)
                print(f"Adjusted similarity: added {abs(min_similarity):.3f} to make all scores positive")
                print(f"Original range: {min_similarity:.3f} - {cosine_similarity(combined_matrix).max():.3f}")
                print(f"Adjusted range: {similarity_matrix.min():.3f} - {similarity_matrix.max():.3f}")
            ...
            print(f"Original similarity range: {cosine_similarity(combined_matrix).min():.3f} - {cosine_similarity(combined_matrix).max():.3f}")
            print(f"Adjusted similarity range: {similarity_matrix.min():.3f} - {similarity_matrix.max():.3f}")
            print("All similarity scores are now positive (>= 0)")
            return True
        except Exception as e:
